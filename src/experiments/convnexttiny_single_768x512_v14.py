# -*- coding: utf-8 -*-
# =========================================================================
# 손 X-ray 골연령 추정 - ConvNeXt-Tiny + LDL + FiLM + 수근골 ROI  [v14]
#
# 베이스: v13 (EPOCHS 100). ROI 2-스트림만 추가한 변종이므로
#         v13 과 직접 비교하면 ROI 의 순수 기여분이 나옵니다.
#
# ── v13 대비 변경점 (1가지) ───────────────────────────────────────────
#   수근골 ROI 2-스트림
#     스트림 A  손 전체 768x512   -> 전역 형태 (12~16세에 강함)
#     스트림 B  손목 ROI 384x384  -> 수근골 미세구조 (4~12세 목표)
#     백본 가중치를 공유하고 두 GAP 특징을 concat -> 융합 FC -> 헤드.
#
#     근거: 4~12세가 test 표본의 50% 인데 오차의 62% 를 냅니다.
#           손실 가중을 두 번 걸었지만 4-8y σ 는 6.41 -> 6.31 로 거의
#           움직이지 않았습니다(v9 계단 1.4 / v12 가우시안 1.35).
#           주의 배분이 아니라 정보량 문제이며, 남은 수단은 해상도 재배분입니다.
#
#     비용: 파라미터는 융합 FC 만 증가(백본 공유) -> 배포 무게 거의 그대로.
#           연산은 약 1.37배 (ROI 화소수가 전체의 37%).
#
#     [주의] 학습 전 반드시 qc_wrist_roi.png 로 ROI 가 실제 손목을 잡는지
#            눈으로 확인하세요. 전완만 잡히거나 손가락이 들어오면 무의미합니다.
#
# ── v12 -> v13 에서 들어온 변경점 ─────────────────────────────────────
# ── v12 대비 변경점 (1가지) ───────────────────────────────────────────
#   EPOCHS 40 -> 100 (warmup 3 -> 5)
#     근거: v11 의 best 가 마지막 에폭(40/40)에서 나왔고 ep37/38 에서도
#           연속 갱신됐습니다. 스케줄에 막혀 조기 종료된 형태입니다.
#             v10 (무효했던 60ep):  best ep34/60, ep54~60 완전 평평
#             v11 (이번 근거):      best ep40/40, 마지막까지 갱신 중
#           FiLM 은 0 초기화라 맨바닥에서 자라납니다. 백본만 사전학습 상태이므로
#           FiLM 이 제 역할을 하려면 더 긴 학습이 필요합니다.
#     [주의] 코사인이 100ep 에 맞춰 감기므로 단순한 '연장' 이 아니라
#            LR 궤적 자체가 다른 실험입니다. v12 와 직접 비교 가능합니다.
#
# ── 참고 성적 ─────────────────────────────────────────────────────────
#   v11  test MAE 4.05 RMSE 5.10 CI95 [3.64, 4.51] | val 5.78 / 7.99  <- 최고
#   v12  test MAE 4.16 RMSE 5.21 CI95 [3.73, 4.61] | val 5.78 / 7.98
#   * 두 CI 는 크게 겹칩니다. test N=197 에서는 무승부입니다.
#
# ── v11 -> v12 에서 들어온 변경점 ─────────────────────────────────────
# ── v11 대비 변경점 (3가지) ───────────────────────────────────────────
#   1. 회전 증강  12° -> 5°   [오류 수정]
#      letterbox 는 손을 캔버스에 딱 맞추므로 회전 여유가 0 입니다.
#      768x494 손 기준 실측 잘림량:
#          12° -> 세로 86px / 가로 131px   (위아래 43px 씩 = 손목·손끝 소실)
#           5° -> 세로 40px / 가로  47px   (AUG_SCALE 축소분으로 흡수)
#      손목이 잘리면 수근골이 사라지는데, 그것이 4~12세 판독의 핵심 단서입니다.
#      4~8세 σ 가 두 번의 손실가중(1.4 / 1.35)에도 내려가지 않은 것과
#      무관하지 않을 수 있습니다.
#   2. 좌우 반전 증강 신규 (p=0.5)   [근거 오류 수정]
#      v9~v11 주석의 "RSNA 는 전부 왼손" 은 실무적으로 틀린 서술이었습니다.
#      촬영 부위는 왼손이지만 PA/AP 자세에 따라 화면상 거울상이 되고,
#      DICOM 변환 반전·오른손 혼입까지 더해 두 방향이 공존합니다.
#      골성숙은 좌우 대칭이므로 반전이 골연령 정보를 훼손하지 않습니다.
#      + QC 에 좌우(엄지) 방향 분포 집계와 검증 시트를 추가했습니다.
#   3. 평가에 예측 분포 왜도 진단 추가
#      MAE 는 분포를 기대값으로 접은 뒤 계산되므로, 대칭 분포에서는 폭이
#      변해도 기대값이 거의 안 움직입니다(폭 10 -> 119.00 / 폭 34 -> 119.01).
#      v10 의 λ 상향이 무효였던 이유가 이것입니다. 기대값을 미는 것은
#      폭이 아니라 비대칭이므로, 왜도를 재서 다음 방향을 결정합니다.
#
# ── v10 실험 결론 (원복 근거) ─────────────────────────────────────────
...
# │            3. fit_canvas      비율유지 리사이즈 + 0 패딩 → 768×512   │
# │            ▼                                                         │
# │      [F] 캐시  cache_convnext_single_v14/<PRE_TAG>/  ← v14 전용      │
# │            ▼                                                         │
...
#
# ▶ 실행: python convnexttiny_single_768x512_v14.py
...
#
# ▶ 산출물  checkpoints_convnext_single_v14_ldl/  (USE_LDL=False 면 _scalar)
...
#   logs/convnext_single_v14_<ts>.log   실행 로그 전체
# =========================================================================
...
RUN_STATE = LOG_DIR / "convnext_single_v14_running.json"
_WORKER_ENV = "BONEAGE_CONVNEXT_SINGLE_V14_WORKER"
...
def _spawn_detached():
    ...
    log_path = LOG_DIR / f"convnext_single_v14_{ts}.log"
...
HAND_CROP_DIR = BASE_DIR / "crop_yolo_seg"          # train / validation / test 하위폴더
CSV_DIR       = BASE_DIR / "crop_yolo_seg" / "csv"  # train.csv / validation.csv / test.csv
...
IMG_H, IMG_W = 768, 512   # [v9] 640x448 -> 수근골 유효화소 +44%
...
# ---------------------------------------------------------------- [v14 신규]
# 수근골 ROI 2-스트림
#   근거: 4~12세가 test 표본의 50% 인데 오차의 62% 를 냅니다. 이 구간의
#         판독 단서는 손목 수근골의 미세구조인데, 768x512 전체 프레임에서
#         해당 영역은 대략 90x90 픽셀에 불과합니다.
#         손실 가중을 두 번(v9 1.4 계단 / v12 1.35 가우시안) 걸었지만
#         4-8y σ 는 6.41 -> 6.31 로 거의 움직이지 않았습니다.
#         학습 자원을 더 줘도 안 되는 것은 '주의 배분' 이 아니라 '정보량'
#         문제라는 뜻이고, 남은 유일한 수단이 해상도 재배분입니다.
#
#   방법: 같은 백본(가중치 공유)으로 두 번 통과시킵니다.
#           스트림 A  손 전체 768x512  -> 전역 형태 (12~16세에 강함)
#           스트림 B  손목 ROI 384x384 -> 수근골 미세구조 (4~12세 목표)
#         두 GAP 특징을 concat 해서 헤드로 넘깁니다.
#
#   비용: 파라미터는 융합 FC 만 늘어 배포 무게가 거의 그대로입니다
#         (백본 가중치 공유). 연산은 ROI 화소수가 전체의 37% 이므로 약 1.37배.
#
#   ROI 추출: 별도 검출기 없이 정규화된 입력 텐서에서 직접 잘라냅니다.
#         패딩 화소가 PAD_NORM 상수로 고정돼 있어 손 마스크를 그대로 얻을 수
#         있습니다. 실패 시 전체 프레임으로 폴백하고 카운터에 집계합니다.
USE_WRIST_ROI = True
ROI_SIZE      = 384     # ROI 입력 한 변 (정사각)
ROI_BAND      = 0.32    # 손 세로 길이의 하단 몇 %를 손목으로 볼지
ROI_PAD_X     = 0.10    # ROI 좌우 여유 비율
...
EPOCHS        = 100         # [v13] 40 -> 100.
                            #   v10 에서 60ep 이 무효였던 것과 상황이 다릅니다.
                            #     v10: best ep34/60, ep54~60 완전 평평  -> 낭비
                            #     v11: best ep40/40 (마지막 에폭), ep37/38 에서도
                            #          연속 갱신 -> 스케줄에 막혀 조기 종료된 형태
                            #   FiLM 파라미터는 0 초기화라 사실상 0 에서 자라납니다.
                            #   백본은 사전학습 상태인데 FiLM 만 맨바닥이므로
                            #   수렴에 더 많은 에폭이 필요합니다.
                            #   [주의] 코사인이 100ep 에 맞춰 감기므로 중간 지점의
                            #   LR 이 40ep 때보다 높습니다. 단순히 '더 오래' 가
                            #   아니라 스케줄 자체가 달라진 실험입니다.
                            #   소요: 약 7.8분/ep x 100 = 약 13시간
LR_HEAD       = 1e-4
LR_BACKBONE   = 4e-5        # 사전학습 특징 보존 -> 헤드보다 낮게
WEIGHT_DECAY  = 0.05        # AdamW. norm/bias 에는 적용하지 않음
WARMUP_EPOCHS = 5           # [v13] EPOCHS 100 에 맞춰 3 -> 5 (비율 유지)
...
USE_AUG       = True
AUG_ROT_DEG   = 5           # [v12] 12 -> 5. letterbox 는 손을 캔버스에 딱 맞추므로
                            #   회전 여유가 0 입니다. 768x494 손 기준 실측 잘림량:
                            #      12°: 세로 86px / 가로 131px  <- 손목(수근골) 소실
                            #       7°: 세로 55px / 가로  72px
                            #       5°: 세로 40px / 가로  47px  (AUG_SCALE 축소로 흡수)
                            #   실제 촬영 각도 편차도 ±5~10° 수준입니다.
AUG_TRANSLATE = 0.10        # [v9] 크롭 중심 이동 내성
AUG_SCALE     = (0.80, 1.15)  # [v12] 하한 0.82 -> 0.80 (회전 여유 확보)
                              #       상한 1.22 -> 1.15 (확대와 회전 잘림이 겹치는 것 방지)
                              #   검출기/크롭마진 차이는 여전히 여기로 일원화합니다.
AUG_SHEAR     = 4
...
# ---------------------------------------------------------------- [v12 신규]
# 좌우 반전 증강
#   [정정] v9~v11 주석의 "RSNA 는 전부 왼손이므로 반전 금지" 는 틀린 서술입니다.
#   촬영 부위는 왼손이 맞지만 화면상 방향은 두 가지로 갈립니다.
#       PA (손바닥을 판에 대고 촬영) : 손등이 보임
#       AP (손등을 판에 대고 촬영)   : 손바닥이 보임 -> 완전한 거울상
#   여기에 DICOM 변환 중 반전, 실제 오른손 혼입까지 더해져 데이터셋 안에
#   두 방향이 공존합니다. 따라서 '데이터가 균질하므로 반전은 OOD 입력' 이라는
#   기존 배제 근거가 성립하지 않습니다.
#   골성숙은 좌우 대칭이므로 반전이 골연령 정보를 훼손하지도 않습니다.
#
#   [주의] TTA 에는 넣지 않습니다. 증강은 불변성 학습, TTA 는 분산 감소로
#   목적이 다르며, 반전 TTA 는 예측을 평균 쪽으로 끌어당깁니다.
AUG_HFLIP_P   = 0.5
...
# ── 추론 ─────────────────────────────────────────────────────────────
TTA_ANGLES = (0, -6, -3, 3, 6)   # [v9] 3 -> 5.
                                 # [v12] 좌우반전 TTA 는 넣지 않습니다. 데이터에 두
                                 #   방향이 공존하는 것은 맞지만, TTA 의 목적은 분산
                                 #   감소이지 불변성 학습이 아닙니다. 반전 TTA 는
                                 #   서로 다른 두 예측을 평균내 결과를 중앙으로
                                 #   끌어당깁니다(수축 악화). 불변성은 학습 단계의
                                 #   AUG_HFLIP_P 가 담당합니다.
...
CACHE_DIR = BASE_DIR / "cache_convnext_single_v14" / PRE_TAG
#   체크포인트는 헤드 shape 가 다르므로 반드시 분리 (스칼라 240-way 충돌 방지)
CKPT_DIR  = BASE_DIR / ("checkpoints_convnext_single_v14_ldl" if USE_LDL
                        else "checkpoints_convnext_single_v14_scalar")
...
print("=" * 72)
log("ConvNeXt-Tiny + LDL 기대값 회귀 골연령 시작 [v14]"
    if USE_LDL else "ConvNeXt-Tiny 스칼라 회귀 골연령 시작 [v14]")
...
log(f"[v14] 회전 {AUG_ROT_DEG}° · 스케일 {AUG_SCALE} · 좌우반전 p={AUG_HFLIP_P}")
log(f"[v14] EPOCHS {EPOCHS} · warmup {WARMUP_EPOCHS} · 예상 소요 약 {EPOCHS*10.7/60:.0f}시간 (ROI 로 약 1.37배)")
log(f"[v14] 수근골 ROI " + (f"ON {ROI_SIZE}x{ROI_SIZE} · band {ROI_BAND} · padX {ROI_PAD_X}"
                          if USE_WRIST_ROI else "OFF"))
log(f"[v14] 평가 리포트: 성별 분해 · 성별x연령 교차 · 성별 플립 · 분포 형태(왜도)")
...
def detect_laterality(g):
    """[v12] 화면상 엄지 방향 판정.  +1 = 엄지가 오른쪽 / -1 = 왼쪽 / 0 = 판정불가

    엄지는 손바닥 옆으로 크게 뻗어 나옵니다. 손가락 영역(상단)의 수평 중심을
    기준으로, 손바닥 영역(중단)에서 좌/우 최대 도달 거리를 비교합니다.

    무게중심 비교가 아니라 '최대 도달 거리' 를 쓰는 이유:
      손바닥이 워낙 커서 무게중심은 엄지 신호를 희석시킵니다. 합성 검증에서
      얇은 엄지(26px)일 때 무게중심 방식은 판정에 실패했지만(편차 8.6px,
      임계 10.2px 미달) 최대 도달 거리 방식은 58px 로 뚜렷하게 잡았습니다.

    임계값 미만이면 0 을 반환합니다. 억지로 판정하지 않는 편이 안전합니다.
    """
    try:
        ...
        if h < 50:
            return 0
        fing = m[y0:y0 + int(0.30 * h)]                      # 상단 30% = 손가락
        palm = m[y0 + int(0.32 * h):y0 + int(0.75 * h)]      # 중단 = 손바닥 + 엄지
        wf = fing.sum(0).astype(np.float64)
        wp = palm.sum(0)
        if wf.sum() < 1 or wp.sum() < 1:
            return 0
        cf = float(np.average(np.arange(m.shape[1], dtype=np.float64), weights=wf))
        cols = np.nonzero(wp)[0]
        d = (float(cols.max()) - cf) - (cf - float(cols.min()))
        return int(np.sign(d)) if abs(d) > 0.03 * m.shape[1] else 0
    except Exception:
        return 0


def qc_laterality(n=300):
    """[v12] 좌우(엄지) 방향 분포를 집계.

    PA/AP 촬영자세에 따라 같은 왼손도 거울상으로 나타납니다. 한쪽으로
    완전히 쏠려 있다면 반전 증강의 이득이 작고, 섞여 있다면 큽니다.
    판정 정확도는 qc_laterality.png 로 반드시 눈으로 검증하세요.
    """
    log("[QC] 좌우(엄지) 방향 분포 집계")
    sample = train_df.sample(min(n, len(train_df)), random_state=SEED)
    lab, paths = [], []
    for _, r in sample.iterrows():
        # train_df 는 train split 이므로 캐시 경로가 고정입니다 (qc_sheet 와 동일 규칙)
        g = imread_kr(CACHE_DIR / "train" / f"{r['id']}.png", cv2.IMREAD_GRAYSCALE)
        if g is None:
            g = imread_kr(r["path"], cv2.IMREAD_GRAYSCALE)   # 캐시 실패 시 원본으로 폴백
        if g is None:
            continue
        lab.append(detect_laterality(g)); paths.append((r["id"], g))
    if not lab:
        log("[QC] 좌우 판정 샘플을 읽지 못했습니다 - 건너뜁니다.")
        return
    lab = np.asarray(lab)
    n_r, n_l, n_u = int((lab == 1).sum()), int((lab == -1).sum()), int((lab == 0).sum())
    tot = len(lab)
    log(f"[QC] 엄지 오른쪽 {n_r} ({100*n_r/tot:.1f}%) | 왼쪽 {n_l} ({100*n_l/tot:.1f}%) "
        f"| 판정불가 {n_u} ({100*n_u/tot:.1f}%)")
    minor = min(n_r, n_l) / max(1, n_r + n_l)
    if minor > 0.10:
        log(f"[QC] 두 방향이 혼재합니다 (소수쪽 {100*minor:.1f}%) "
            f"-> 좌우 반전 증강(AUG_HFLIP_P={AUG_HFLIP_P})이 유효합니다.")
    else:
        log(f"[QC] 한쪽으로 쏠려 있습니다 (소수쪽 {100*minor:.1f}%) "
            f"-> 반전 증강 이득이 작을 수 있습니다. 결과 비교 시 참고하세요.")

    # 판정 결과를 눈으로 검증할 시트 (각 라벨별 4장)
    try:
        pick, per = [], {1: [], -1: [], 0: []}
        for (i, g), v in zip(paths, lab):
            if len(per[v]) < 4:
                per[v].append((i, g))
        for v in (1, -1, 0):
            pick += [(v, i, g) for i, g in per[v]]
        if pick:
            fig, ax = plt.subplots(1, len(pick), figsize=(2.2 * len(pick), 4.2))
            ax = np.atleast_1d(ax)
            for a, (v, i, g) in zip(ax, pick):
                a.imshow(g, cmap="gray"); a.axis("off")
                a.set_title({1: "엄지 오른쪽", -1: "엄지 왼쪽", 0: "판정불가"}[v], fontsize=9)
            plt.tight_layout()
            pth = CKPT_DIR / "qc_laterality.png"
            plt.savefig(pth, dpi=110); plt.close()
            log(f"[QC] 좌우 판정 시트 저장: {pth}  <- 판정이 맞는지 반드시 눈으로 확인하세요")
    except Exception as e:
        log(f"[QC] 좌우 판정 시트 생성 실패(무시 가능): {e}")


def qc_wrist_roi(n=12):
    """[v14] ROI 가 실제로 손목(수근골)을 잡는지 눈으로 검증할 시트.

    ROI 는 학습 파이프라인과 완전히 동일한 경로(정규화 텐서 -> extract_wrist_roi)로
    뽑습니다. 여기서 전완만 잡히거나 손가락이 들어오면 ROI 스트림은 무의미하므로
    학습 전에 반드시 확인해야 합니다.
    """
    if not USE_WRIST_ROI:
        return
    log("[QC] 수근골 ROI 검증 시트 생성")
    sample = train_df.sample(min(n, len(train_df)), random_state=SEED + 1)
    xs, ids = [], []
    for _, r in sample.iterrows():
        g = imread_kr(CACHE_DIR / "train" / f"{r['id']}.png", cv2.IMREAD_GRAYSCALE)
        if g is None:
            continue
        xs.append(eval_tf(np.stack([g] * 3, axis=-1)))
        ids.append(r["id"])
    if not xs:
        log("[QC] ROI 샘플을 읽지 못했습니다 - 건너뜁니다.")
        return
    x = torch.stack(xs)
    roi, fails = extract_wrist_roi(x)
    log(f"[QC] ROI 추출 실패 {fails}/{len(xs)} 건")

    def denorm(t):
        m = torch.tensor(IMAGENET_MEAN).view(3, 1, 1)
        sd = torch.tensor(IMAGENET_STD).view(3, 1, 1)
        return (t * sd + m).clamp(0, 1).permute(1, 2, 0).numpy()

    k = len(xs)
    fig, axes = plt.subplots(2, k, figsize=(1.9 * k, 6.4))
    axes = np.atleast_2d(axes)
    for i in range(k):
        axes[0, i].imshow(denorm(x[i])); axes[0, i].axis("off")
        axes[0, i].set_title(str(ids[i])[:10], fontsize=7)
        axes[1, i].imshow(denorm(roi[i])); axes[1, i].axis("off")
    axes[0, 0].set_ylabel("전체", fontsize=9)
    axes[1, 0].set_ylabel("ROI", fontsize=9)
    plt.tight_layout()
    pth = CKPT_DIR / "qc_wrist_roi.png"
    plt.savefig(pth, dpi=110); plt.close()
    log(f"[QC] ROI 시트 저장: {pth}")
    log("[QC] 아랫줄이 손목(수근골)인지 반드시 눈으로 확인하세요. "
        "전완만 잡히거나 손가락이 들어오면 ROI_BAND 를 조정해야 합니다.")


qc_sheet()
qc_laterality()
# qc_wrist_roi() 는 eval_tf / extract_wrist_roi 정의 이후에 호출합니다 ([I] 모델 절 끝)
if QC_ONLY and not USE_WRIST_ROI:
...
_aug = [
    transforms.RandomHorizontalFlip(p=AUG_HFLIP_P),   # [v12] PA/AP 촬영자세 차이 흡수
...
def extract_wrist_roi(x, size=None, band=None, pad_x=None):
    """[v14] 정규화된 입력 텐서에서 손목(수근골) 영역만 잘라 정사각으로 리사이즈.

    입력  x  (B, 3, H, W)  ImageNet 정규화 완료 텐서
    반환     (B, 3, S, S), 실패건수(int)

    letterbox 패딩 화소는 정규화 후 PAD_NORM 상수로 고정되므로, 그 값보다
    충분히 큰 화소를 손으로 봅니다(별도 검출기 불필요). 손 세로 범위를 구한 뒤
    하단 band 비율을 손목으로 잡고, 그 구간의 좌우 실제 범위에 여유를 더합니다.

    실패(마스크가 거의 비어 있음)하면 그 샘플만 전체 프레임을 리사이즈해
    돌려주고 실패 카운터를 올립니다. 조용히 잘못된 영역을 넘기지 않습니다.
    """
    S = int(ROI_SIZE if size is None else size)
    bd = float(ROI_BAND if band is None else band)
    px = float(ROI_PAD_X if pad_x is None else pad_x)
    B, C, H, W = x.shape
    thr = torch.as_tensor(PAD_NORM, device=x.device, dtype=x.dtype).view(1, C, 1, 1) + 0.15
    m = (x > thr).any(1)                                   # (B, H, W) 손 마스크
    outs, fails = [], 0
    for i in range(B):
        rows = torch.nonzero(m[i].any(1), as_tuple=False).flatten()
        if rows.numel() < 8:
            outs.append(F.interpolate(x[i:i + 1], size=(S, S),
                                      mode="bilinear", align_corners=False))
            fails += 1
            continue
        y0, y1 = int(rows[0]), int(rows[-1])
        hh = max(1, y1 - y0)
        ty = max(0, int(y1 - bd * hh))                      # 손목 밴드 상단
        cols = torch.nonzero(m[i, ty:y1 + 1].any(0), as_tuple=False).flatten()
        if cols.numel() < 8:
            outs.append(F.interpolate(x[i:i + 1], size=(S, S),
                                      mode="bilinear", align_corners=False))
            fails += 1
        ...
        pad = int(px * max(1, x1 - x0))
        x0 = max(0, x0 - pad); x1 = min(W - 1, x1 + pad)
        crop = x[i:i + 1, :, ty:y1 + 1, x0:x1 + 1]
        outs.append(F.interpolate(crop, size=(S, S), mode="bilinear", align_corners=False))
    return torch.cat(outs, dim=0), fails
...
class ConvNeXtRegressor(nn.Module):
    ...
    def __init__(self, backbone_name=BACKBONE, img_hw=None, head_type=HEAD_TYPE,
                 ...
                 film_hidden=None, use_roi=None, roi_size=None):
        super().__init__()
        ...
        # ---- [v14] 수근골 ROI 2-스트림 -------------------------------
        _ur = USE_WRIST_ROI if use_roi is None else bool(use_roi)
        self.use_roi = bool(_ur) and head_type == "gap"     # gap 헤드에서만 지원
        self.roi_size = int(ROI_SIZE if roi_size is None else roi_size)
        if _ur and not self.use_roi and verbose:
            log("[head] ROI 비활성 - head_type='gap' 에서만 지원합니다")
        if self.use_roi and verbose:
            log(f"[head] 수근골 ROI 2-스트림 ON · ROI {self.roi_size}x{self.roi_size} "
                f"(백본 가중치 공유, 화소비 {self.roi_size**2/(h*w)*100:.0f}%)")
        ...
        if head_type == "gap":
            ...
            img_out = head_dim
            if self.use_roi:
                # ROI 스트림 전용 정규화/투영. 백본은 공유하지만 통계가 다르므로
                # 헤드는 분리합니다. 융합 후 차원은 head_dim 으로 되돌립니다.
                self.norm_roi = nn.LayerNorm(C)
                self.proj_roi = nn.Sequential(nn.Linear(C, head_dim), nn.GELU(),
                                              nn.Dropout(dropout))
                self.fuse = nn.Sequential(nn.Linear(head_dim * 2, head_dim), nn.GELU(),
                                          nn.Dropout(dropout))
                img_out = head_dim
    ...
    def forward(self, x, g):
        ...
        if self.head_type == "gap":
            ...
            if self.use_roi:
                xr, nfail = extract_wrist_roi(x, size=self.roi_size)
                self.roi_fail = int(nfail)                  # 진단용 카운터
                fr = self._backbone_forward(xr, g)          # 백본 가중치 공유
                zr = F.adaptive_avg_pool2d(fr, 1).flatten(1)
                zr = self.proj_roi(self.norm_roi(zr))
                z = self.fuse(torch.cat([z, zr], dim=1))
        else:
...
qc_wrist_roi()          # [v14] eval_tf / extract_wrist_roi 준비 완료 후 실행
if QC_ONLY:
    log("--qc-only: QC 시트만 만들고 종료합니다.")
    raise SystemExit(0)
...
def build_model(arch, pretrained=False, verbose=True):
    ...
    m = ConvNeXtRegressor(
        ...
        use_roi=arch.get("USE_ROI", False),       # 구버전 arch 에는 ROI 가 없음
        roi_size=arch.get("ROI_SIZE", 384),
    )
...
ARCH = {"BACKBONE": BACKBONE, "IMG_H": IMG_H, "IMG_W": IMG_W,
        ...
        "USE_ROI": USE_WRIST_ROI, "ROI_SIZE": ROI_SIZE,
        "ROI_BAND": ROI_BAND, "ROI_PAD_X": ROI_PAD_X,
        "AUG_ROT_DEG": AUG_ROT_DEG, "AUG_SCALE": list(AUG_SCALE),
...
def dist_shape_stats(model, df, split, bs=16):
    """[v12] 예측 분포의 폭과 '왜도(비대칭)' 를 집계. LDL 모드 전용.

    왜 왜도인가:
      MAE 는 분포를 기대값 하나로 접은 뒤 계산되는데, 좌우 대칭 분포에서는
      폭이 아무리 달라져도 기대값이 거의 움직이지 않습니다.
          폭 10 -> 기대값 119.00 / 폭 34 -> 기대값 119.01
      v10 에서 λ 를 올려 폭을 34.2 -> 21.8 로 조였는데 MAE 가 개선되지 않은
      이유가 이것입니다. 기대값을 밀어내는 것은 폭이 아니라 비대칭입니다.
      따라서 RMSE 꼬리의 원인이 '분포 비대칭' 인지 '개별 실패 케이스' 인지
      가르려면 폭이 아니라 왜도를 봐야 합니다.
    """
    if not USE_LDL:
        return None
    loader = DataLoader(BoneAgeDataset(df, split, eval_tf), batch_size=bs,
    ...
    with torch.no_grad():
        for x, g, _, ym in loader:
            x = x.to(device).to(memory_format=torch.channels_last); g = g.to(device)
            logits = model(x, g).float()                 # AMP 밖 fp32
            if k is None:
                k = torch.arange(1, logits.size(1) + 1, device=logits.device,
                                 dtype=torch.float32)
            pr = torch.softmax(logits, dim=1)
            mu = (pr * k).sum(1)
            var = (pr * (k - mu[:, None]) ** 2).sum(1)
            skew = (pr * (k - mu[:, None]) ** 3).sum(1) / var.clamp_min(1e-9) ** 1.5
            SD.append(var.sqrt().cpu()); SK.append(skew.cpu())
            MU.append(mu.cpu())
            # ym 은 (B,1) 로 배치됩니다. 평탄화하지 않으면 뒤에서
            # (N,) - (N,1) 브로드캐스트로 (N,N) 이 되어 터집니다.
            T.append(ym.float().reshape(-1))
    sd = torch.cat(SD).numpy().reshape(-1); sk = torch.cat(SK).numpy().reshape(-1)
    mu = torch.cat(MU).numpy().reshape(-1); t = torch.cat(T).numpy().reshape(-1)
    assert sd.shape == t.shape == mu.shape, (sd.shape, mu.shape, t.shape)
    err = mu - t
    big = np.abs(err) > 15.0                              # RMSE 를 지배하는 큰 오차
    out = {"n": int(len(sd)),
           "width_mean": round(float(sd.mean()), 2),
           "skew_mean": round(float(sk.mean()), 3),
           "skew_abs_mean": round(float(np.abs(sk).mean()), 3),
           "skew_gt05_pct": round(float(np.mean(np.abs(sk) > 0.5) * 100), 1),
           "n_big_err": int(big.sum()),
           "skew_abs_big": round(float(np.abs(sk[big]).mean()), 3) if big.any() else None,
           "skew_abs_rest": round(float(np.abs(sk[~big]).mean()), 3) if (~big).any() else None,
           "width_big": round(float(sd[big].mean()), 2) if big.any() else None,
           "width_rest": round(float(sd[~big].mean()), 2) if (~big).any() else None}
    return out
...
def gender_flip_test(model, df, split, bs=16):
    ...
    with torch.no_grad():
        for x, g, _, ym in loader:
            ...
            D.append((p1 - p0).float().reshape(-1).cpu())
            T.append(ym.float().reshape(-1))   # (B,1) -> (B,)
    d = torch.cat(D).numpy().reshape(-1); t = torch.cat(T).numpy().reshape(-1)
    assert d.shape == t.shape, (d.shape, t.shape)
    rows = [{"구간": "전체", "N": len(d),
...
# ── [v12] 예측 분포 형태 진단 ───────────────────────────────────────
try:
    _ds = dist_shape_stats(eval_model, val_df, "val")
    if _ds:
        lines.append("-" * 68)
        lines.append(" [예측 분포 형태 · val]")
        lines.append(f"   폭(표준편차) 평균 {_ds['width_mean']:.2f}개월  (목표 δ={LDL_DELTA})")
        lines.append(f"   왜도 평균 {_ds['skew_mean']:+.3f} · |왜도| 평균 {_ds['skew_abs_mean']:.3f} "
                     f"· |왜도|>0.5 비율 {_ds['skew_gt05_pct']:.1f}%")
        if _ds["skew_abs_big"] is not None:
            lines.append(f"   큰오차(>15개월) {_ds['n_big_err']}건: |왜도| {_ds['skew_abs_big']:.3f} "
                         f"/ 폭 {_ds['width_big']:.2f}")
            lines.append(f"   나머지          : |왜도| {_ds['skew_abs_rest']:.3f} "
                         f"/ 폭 {_ds['width_rest']:.2f}")
            _r = _ds["skew_abs_big"] / max(_ds["skew_abs_rest"], 1e-9)
            lines.append("   판정: " + ("큰오차의 비대칭이 뚜렷 -> 분포 비대칭 억제가 유효 "
                                       "(KL 대신 EMD/Wasserstein 검토)" if _r > 1.3
                                       else "비대칭 차이 미미 -> 분포가 아니라 개별 이미지 문제 "
                                            "(worst_cases.png 확인)"))
        results["dist_shape_val"] = _ds
except Exception as _e:
    log(f"  [경고] 분포 형태 진단 실패: {_e}")
