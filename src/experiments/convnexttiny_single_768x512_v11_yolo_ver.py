# -*- coding: utf-8 -*-
# =========================================================================
# 손 X-ray 골연령 추정 - ConvNeXt-Tiny + LDL + FiLM 성별조건화  [v11]
#
# 베이스: v9 설정 (v10 의 λ/EPOCHS 변경은 무효로 판명되어 원복).
#
# ── v10 실험 결론 (원복 근거) ─────────────────────────────────────────
...
# │            ▼                                                         │
# │      [F] 캐시  cache_convnext_single_v11/<PRE_TAG>/  ← v11 전용      │
# │            ▼                                                         │
...
#
# ▶ 실행: python convnexttiny_single_768x512_v11.py
...
#
# ▶ 산출물  checkpoints_convnext_single_v11_ldl/  (USE_LDL=False 면 _scalar)
...
#   logs/convnext_single_v11_<ts>.log   실행 로그 전체
# =========================================================================
...
RUN_STATE = LOG_DIR / "convnext_single_v11_running.json"
_WORKER_ENV = "BONEAGE_CONVNEXT_SINGLE_V11_WORKER"
...
def _spawn_detached():
    ...
    log_path = LOG_DIR / f"convnext_single_v11_{ts}.log"
...
CACHE_DIR = BASE_DIR / "cache_convnext_single_v11" / PRE_TAG
#   체크포인트는 헤드 shape 가 다르므로 반드시 분리 (스칼라 240-way 충돌 방지)
CKPT_DIR  = BASE_DIR / ("checkpoints_convnext_single_v11_ldl" if USE_LDL
                        else "checkpoints_convnext_single_v11_scalar")
...
print("=" * 72)
log("ConvNeXt-Tiny + LDL 기대값 회귀 골연령 시작 [v11]"
    if USE_LDL else "ConvNeXt-Tiny 스칼라 회귀 골연령 시작 [v11]")
