# -*- coding: utf-8 -*-
"""
손목 절단선 정렬  [B안 - 회전 + 절단]  crop_yolo_seg -> crop_wristline_cut

무엇을 하나
    마스크 하단에서 '손목 절단선'(첨부 그림의 파란선)을 자동으로 찾아
    그 선이 수평이 되도록 회전시킨 뒤, 그 선 아래를 전부 잘라냅니다.
    결과 이미지의 아래 모서리가 곧 손목 절단선이 되고, 그 아래의 흰 띠
    (촬영판/차폐물)는 사라집니다.

    A안(make_wristline_rot.py)과 회전 각도는 완전히 동일합니다.
    다른 것은 CUT_BELOW 하나뿐이라 두 결과를 그대로 비교할 수 있습니다.
...
사용법
    아래 [설정] 블록만 맞추고 실행합니다.
        python make_wristline_cut.py
    다시 만들려면
        python make_wristline_cut.py --overwrite
"""
import argparse
import json
import sys
...
DST_DIR   = BASE_DIR / "crop_wristline_cut"      # 출력  [B안 - 회전 + 절단]
...
CUT_BELOW = True         # [B안] True = 절단선 아래를 잘라냄
BOTTOM_PAD = 6           # 절단선 아래로 남길 여유 화소. 0 이면 딱 붙습니다
