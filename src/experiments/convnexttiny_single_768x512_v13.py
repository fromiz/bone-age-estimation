# -*- coding: utf-8 -*-
# =========================================================================
# 손 X-ray 골연령 추정 - ConvNeXt-Tiny + LDL + FiLM  [v13]
#
# 베이스: v12. 학습 길이만 늘린 변종입니다 (EPOCHS 40 -> 100).
#
# ── v12 대비 변경점 (1가지) ───────────────────────────────────────────
...
# │            ▼                                                         │
# │      [F] 캐시  cache_convnext_single_v13/<PRE_TAG>/  ← v13 전용      │
# │            ▼                                                         │
...
#
# ▶ 실행: python convnexttiny_single_768x512_v13.py
...
#
# ▶ 산출물  checkpoints_convnext_single_v13_ldl/  (USE_LDL=False 면 _scalar)
...
#   logs/convnext_single_v13_<ts>.log   실행 로그 전체
# =========================================================================
...
RUN_STATE = LOG_DIR / "convnext_single_v13_running.json"
_WORKER_ENV = "BONEAGE_CONVNEXT_SINGLE_V13_WORKER"
...
def _spawn_detached():
    ...
    log_path = LOG_DIR / f"convnext_single_v13_{ts}.log"
...
CACHE_DIR = BASE_DIR / "cache_convnext_single_v13" / PRE_TAG
#   체크포인트는 헤드 shape 가 다르므로 반드시 분리 (스칼라 240-way 충돌 방지)
CKPT_DIR  = BASE_DIR / ("checkpoints_convnext_single_v13_ldl" if USE_LDL
                        else "checkpoints_convnext_single_v13_scalar")
...
print("=" * 72)
log("ConvNeXt-Tiny + LDL 기대값 회귀 골연령 시작 [v13]"
    if USE_LDL else "ConvNeXt-Tiny 스칼라 회귀 골연령 시작 [v13]")
...
log(f"[v13] 회전 {AUG_ROT_DEG}° · 스케일 {AUG_SCALE} · 좌우반전 p={AUG_HFLIP_P}")
log(f"[v13] EPOCHS {EPOCHS} (v11/v12 40) · warmup {WARMUP_EPOCHS} · 예상 소요 약 {EPOCHS*7.8/60:.0f}시간")
log(f"[v13] 평가 리포트: 성별 분해 · 성별x연령 교차 · 성별 플립 · 분포 형태(왜도)")
