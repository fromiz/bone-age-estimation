...
#     [변경] Phase II 백본 = ConvNeXt-Tiny
#     [유지] R1 밴드 CAM / R2 논문 erase 방식 / 성별 임베딩 / LDL 손실
#     [분리] cache_attention_v8/<PRE_TAG> / checkpoints_attention_v8 / logs/*_v8_*
#
#   ▶ 전처리 체인 (최종)
#       원본(YOLO 크롭) -> [uint8 변환] -> 비율유지 리사이즈 -> CLAHE -> 배경색 패딩
#
#   ▶ 실행: python attention_ldl_boneage_v8_convnext.py
...
RUN_STATE = LOG_DIR / "boneage_attention_v8_running.json"
_WORKER_ENV = "BONEAGE_ATTENTION_V8_WORKER"
...
def _spawn_detached():
    ...
    log_path = LOG_DIR / f"boneage_attention_v8_{ts}.log"
...
CLS_SIZE_REGION = 512     # [v8] 512 -> InceptionV3 특징맵 약 14x14
                          #   (560 대비 CAM 격자가 약간 거칩니다)
REG_SIZE        = 512     # [v8] 560 -> 512 (메모리 -16%, 배치 여유)
...
AGG_CHANNELS = ("hand", "r1", "r2")   # [v8] erased -> r2
...
# =========================================================================
# ① 밝기 정규화
NORM_MODE    = "none"         # [v8] 제거. 타이트 크롭에서 과확장 -> 포화
                              #   되돌리려면 "percentile"
NORM_LO, NORM_HI = 1.0, 99.0  # 백분위 하한/상한 (마커·핫픽셀 클리핑)

# ② Top-Hat  (White Top-Hat = 원본 - Opening(원본))
TOPHAT_MODE   = "none"    # [v8] 제거. add 는 감산항이 없어 골단이 255 절단
...
# =========================================================================
# ── ⓪ 마커 억제 ─────────────────────────────────────────────────────
MARKER_MODE = "off"           # [v8] 제거 (v7 에서 실제 동작 안 함)
...
# ── ⓪-2 회전 정렬 ───────────────────────────────────────────────────
ALIGN_MODE = "off"        # [v8] 제거 (쐐기가 통계/CAM 오염)
...
CACHE_DIR = BASE_DIR / "cache_attention_v8" / PRE_TAG
CKPT_DIR  = BASE_DIR / "checkpoints_attention_v8"
...
print("=" * 64)
log("Attention(R1/R2) + LDL 골연령 - v8 (ConvNeXt-Tiny) 시작")
...
def enhance(gray, return_k=False, return_info=False):
    """[v8] 마커/정규화/회전/Top-Hat 전부 off -> 사실상 uint8 변환만.
...
def make_reg_backbone(pretrained=True):
    """[v8] Phase II 백본. REG_BACKBONE 한 줄로 교체됩니다.
...
# =========================================================================
# -- 하이퍼파라미터 -----------------------------------------------------------
BATCH_SIZE   = 8       # [v8] 512 입력 + ConvNeXt-Tiny. OOM 이면 낮추세요
EPOCHS       = 120     # [논문] 120 epoch
LR_STEPS     = [(60, 1e-4), (90, 3e-5), (120, 1e-5)]   # [v8] 논문 LR 의 1/3
                          #   (현대 백본은 3e-4 에서 발산하기 쉬움)
WEIGHT_DECAY = 0.05    # [v8] AdamW 표준값 (논문은 미언급)
...
if EVAL_ONLY:
    ...
    # [v8] Adam -> AdamW. 현대 백본은 decoupled weight decay 가 표준
    ...
    if AUTO_RESUME and LAST_CKPT.exists():
        ...
        if bad:
            ...
            log("  * 처음부터 다시 하려면 checkpoints_attention_v8 의 last.pt 삭제 후 실행")
    else:
