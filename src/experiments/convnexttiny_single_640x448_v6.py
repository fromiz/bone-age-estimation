# -*- coding: utf-8 -*-
# =========================================================================
# 손 X-ray 골연령 추정 - ConvNeXt-Tiny + 라벨분포학습(LDL) 기대값 회귀
#
# v5(스칼라 회귀) 대비 딱 한 가지만 바뀐 변종입니다.
#   v5 : 마지막 FC -> 1  ·  타깃 z-정규화  ·  L1 손실
#   v6 : 마지막 FC -> 240 -> softmax -> 기대값  ·  타깃 개월 원단위
#        손실 = ℓ_MAE + λ·D_KL(G‖p)      [Chen 2020 arXiv:2006.00202 식 6~12]
#   백본·입력·전처리·증강·EMA·TTA·캘리브레이션은 v5와 완전히 동일하므로
#   v5 vs v6 의 test MAE 차이가 곧 'LDL 손실의 순수 기여분'입니다.
#   (논문 Table V, 원본 이미지 입력 기준: ℓ1 7.3 -> ℓMAE+ℓreg 6.2)
#
# ┌─ 데이터 흐름 ────────────────────────────────────────────────────────┐
# │                                                                      │
...
# │            ▼                                                         │
# │      [F] 캐시  cache_convnext_single_v5/<PRE_TAG>/  ← v5와 공유      │
# │            ▼                                                         │
...
#
# ▶ 실행: python convnexttiny_single_640x448_v6.py
...
#
# ▶ 산출물  checkpoints_convnext_single_v6/
...
#   logs/convnext_single_v6_<ts>.log   실행 로그 전체
# =========================================================================
...
RUN_STATE = LOG_DIR / "convnext_single_v6_running.json"
_WORKER_ENV = "BONEAGE_CONVNEXT_SINGLE_V6_WORKER"
...
def _spawn_detached():
    ...
    log_path = LOG_DIR / f"convnext_single_v6_{ts}.log"
...
CKPT_DIR  = BASE_DIR / ("checkpoints_convnext_single_v6_ldl" if USE_LDL
                        else "checkpoints_convnext_single_v6_scalar")
...
print("=" * 72)
log("ConvNeXt-Tiny + LDL 기대값 회귀 골연령 시작"
    if USE_LDL else "ConvNeXt-Tiny 단일 스칼라 회귀 골연령 시작 (v5 동등)")
