# -*- coding: utf-8 -*-
# =========================================================================
# 손 X-ray 골연령 추정 - ConvNeXt-Tiny + LDL + 도메인 일반화  [v9]
#
# 베이스: v6 (LDL 기대값 회귀). USE_LDL=False 로 두면 v5 스칼라 회귀와 동등.
#
# v6 대비 변경점 (4가지, 전부 [v9] 태그로 표시)
...
# │            ▼                                                         │
# │      [F] 캐시  cache_convnext_single_v9/<PRE_TAG>/  ← v9 전용        │
# │            ▼                                                         │
...
#
# ▶ 실행: python convnexttiny_single_768x512_v9.py
...
#
# ▶ 산출물  checkpoints_convnext_single_v9_ldl/  (USE_LDL=False 면 _scalar)
...
#   logs/convnext_single_v9_<ts>.log   실행 로그 전체
# =========================================================================
...
RUN_STATE = LOG_DIR / "convnext_single_v9_running.json"
_WORKER_ENV = "BONEAGE_CONVNEXT_SINGLE_V9_WORKER"
...
def _spawn_detached():
    ...
    log_path = LOG_DIR / f"convnext_single_v9_{ts}.log"
...
# 연령구간 손실 가중
#   근거: v5 test 기준 4~12세 구간의 오차 SD 가 12~16세의 1.8배이고
#         전체 오차의 65% 를 차지합니다. 표본 수는 오히려 최다이므로
#         샘플러(표본 조작)가 아니라 손실 가중으로 용량을 재배분합니다.
USE_BAND_WEIGHT = True
BAND_EDGES      = (48.0, 144.0)   # 개월. [48,144) = 4~12세
BAND_WEIGHT     = 1.4             # 해당 구간 손실 배율. 1.2 / 1.4 / 1.6 로 튜닝
...
#   [v9] 해상도가 768x512 로 바뀌었으므로 PRE_TAG 가 달라지고,
#   캐시는 v5/v6 과 공유되지 않습니다. 첫 실행 시 약 14,000장을
#   새로 생성합니다 (스테이지1, 약 15분). 이는 의도된 동작입니다.
#   도메인 증강은 런타임 적용이라 PRE_TAG 에 포함하지 않습니다.
CACHE_DIR = BASE_DIR / "cache_convnext_single_v9" / PRE_TAG
#   체크포인트는 헤드 shape 가 다르므로 반드시 분리 (스칼라 240-way 충돌 방지)
CKPT_DIR  = BASE_DIR / ("checkpoints_convnext_single_v9_ldl" if USE_LDL
                        else "checkpoints_convnext_single_v9_scalar")
...
def dump_env(extra=None):
    ...
    info = {
        ...
        "v9": {"domain_aug": USE_DOMAIN_AUG, "jpeg_p": DA_JPEG_P, "gamma_p": DA_GAMMA_P,
               ...
               "band_weight": BAND_WEIGHT, "band_edges": list(BAND_EDGES)},
...
print("=" * 72)
log("ConvNeXt-Tiny + LDL 기대값 회귀 골연령 시작 [v9]"
    if USE_LDL else "ConvNeXt-Tiny 스칼라 회귀 골연령 시작 [v9]")
...
log(f"[v9] 구간가중   " + (f"{BAND_EDGES[0]:.0f}~{BAND_EDGES[1]:.0f}개월 x{BAND_WEIGHT}"
...
def band_weight_of(y_months):
    """[v9] 개월 텐서 -> 구간 손실 가중치 텐서. BAND_EDGES 구간에 BAND_WEIGHT."""
    if not USE_BAND_WEIGHT:
        return torch.ones_like(y_months)
    lo, hi = BAND_EDGES
    return torch.where((y_months >= lo) & (y_months < hi),
                       torch.full_like(y_months, float(BAND_WEIGHT)),
                       torch.ones_like(y_months))
...
ARCH = {"BACKBONE": BACKBONE, "IMG_H": IMG_H, "IMG_W": IMG_W,
        ...
        "DOMAIN_AUG": USE_DOMAIN_AUG, "BAND_WEIGHT": (BAND_WEIGHT if USE_BAND_WEIGHT else 1.0),
...
class LDLExpectationLoss(nn.Module):
    ...
    def forward(self, logits, y_months, w=None):
        """[v9] w: (B,) per-sample 가중치. None 또는 균등이면 v6 와 손실값 동일."""
        logits = logits.float()
    ...
    try:
        for epoch in range(start_epoch, EPOCHS + 1):
            ...
            for it, (x, g, yn, ym) in enumerate(train_loader):
                ...
                with torch.amp.autocast("cuda", enabled=USE_AMP):
                    pred = model(x, g)
                bw = band_weight_of(ym_d)                    # [v9] 구간 손실 가중
                if USE_LDL:
