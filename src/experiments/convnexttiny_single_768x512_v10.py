# -*- coding: utf-8 -*-
# =========================================================================
# 손 X-ray 골연령 추정 - ConvNeXt-Tiny + LDL + 도메인 일반화  [v10]
#
# 베이스: v9. 앙상블 없이 단일 모델로 RMSE 를 줄이는 데 초점을 둡니다.
#
# ── v9 대비 변경점 (3가지) ────────────────────────────────────────────
...
# │            ▼                                                         │
# │      [F] 캐시  cache_convnext_single_v10/<PRE_TAG>/  ← v10 전용      │
# │            ▼                                                         │
...
#
# ▶ 실행: python convnexttiny_single_768x512_v10.py
...
#
# ▶ 산출물  checkpoints_convnext_single_v10_ldl/  (USE_LDL=False 면 _scalar)
...
#   logs/convnext_single_v10_<ts>.log   실행 로그 전체
# =========================================================================
...
RUN_STATE = LOG_DIR / "convnext_single_v10_running.json"
_WORKER_ENV = "BONEAGE_CONVNEXT_SINGLE_V10_WORKER"
...
def _spawn_detached():
    ...
    log_path = LOG_DIR / f"convnext_single_v10_{ts}.log"
...
EPOCHS        = 60          # [v10] 40 -> 60. v9 는 ep34 에서 best 후 평평했으므로
                            #       코사인 꼬리를 늘려 미세 수렴 여지를 확보
LR_HEAD       = 1e-4
LR_BACKBONE   = 4e-5        # 사전학습 특징 보존 -> 헤드보다 낮게
WEIGHT_DECAY  = 0.05        # AdamW. norm/bias 에는 적용하지 않음
WARMUP_EPOCHS = 4           # [v10] 총 에폭 증가에 맞춰 3 -> 4 (비율 유지)
...
LDL_LAMBDA = 1.0    # [v10][추론] 0.5 -> 1.0. v9 의 예측분포 평균 표준편차가
                    #   34.2 개월로 목표 δ=15.0 의 2.3배까지 퍼졌습니다.
                    #   분포가 넓으면 극단 예측이 늘어 RMSE 꼬리가 두꺼워집니다.
                    #   λ 를 올려 분포를 조입니다. 논문 최적구간 [0.1, 1] 의 상단.
                    #   튜닝: age_distribution.png 의 평균 표준편차가 20 근방이 되는 값
...
CACHE_DIR = BASE_DIR / "cache_convnext_single_v10" / PRE_TAG
#   체크포인트는 헤드 shape 가 다르므로 반드시 분리 (스칼라 240-way 충돌 방지)
CKPT_DIR  = BASE_DIR / ("checkpoints_convnext_single_v10_ldl" if USE_LDL
                        else "checkpoints_convnext_single_v10_scalar")
...
print("=" * 72)
log("ConvNeXt-Tiny + LDL 기대값 회귀 골연령 시작 [v10]"
    if USE_LDL else "ConvNeXt-Tiny 스칼라 회귀 골연령 시작 [v10]")
...
log(f"[v10] LDL λ {LDL_LAMBDA} (v9 0.5) · EPOCHS {EPOCHS} (v9 40) · warmup {WARMUP_EPOCHS}")
