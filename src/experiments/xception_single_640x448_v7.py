# -*- coding: utf-8 -*-
# =========================================================================
# 손 X-ray 골연령 추정 - Xception + 라벨분포학습(LDL) 기대값 회귀
#
# v6 와 '백본만' 다른 변종입니다. Chen 2020 이 Table II 에서 VGG19/InceptionV3/
# ResNet50/Xception 중 Xception 을 최종 선택했으므로, 논문 원안 백본으로
# 같은 손실·같은 입력·같은 학습 레시피를 돌려 백본 기여분을 분리합니다.
#
#   v5 : ConvNeXt-Tiny + 스칼라 회귀(L1)        ← 기준선
#   v6 : ConvNeXt-Tiny + LDL 기대값 회귀        ← 손실 효과
#   v7 : Xception      + LDL 기대값 회귀        ← 백본 효과  (이 파일)
#   v8 : v5 + v7 앙상블                         ← 결합 효과
#
# 손실 = ℓ_MAE + λ·D_KL(G‖p)   [Chen 2020 arXiv:2006.00202 식 6~12]
#
# ▶ v6 와 다른 지점 (전부 백본 교체 때문에 따라온 것)
#   1. 화소 정규화 상수: Xception 사전학습 가중치는 ImageNet mean/std 가 아니라
#      Inception 계열의 mean=std=0.5 로 학습됐습니다. 백본 pretrained_cfg 에서
#      런타임에 읽어오므로 수동으로 맞출 필요가 없습니다.
#   2. drop_path: legacy_xception 은 지원하지 않아 자동으로 무시됩니다.
#   3. weight_decay 5e-2 -> 1e-4: 5e-2 는 ConvNeXt 전용 레시피입니다.
#   4. 학습률 3e-4(head): 논문 명시값.
#   5. VRAM: Xception 은 초반 고해상도 단계가 무거워 배치 자동 축소 장치를 넣었습니다.
#
# ┌─ 데이터 흐름 ────────────────────────────────────────────────────────┐
# │                                                                      │
...
# │            ▼                                                         │
# │      [F] 캐시  cache_convnext_single_v5/<PRE_TAG>/  ← v5/v6 과 공유  │
# │            ▼                                                         │
# │      [H] Dataset  캐시 PNG 읽기 → 3채널 복제 → 증강 → ImageNet 정규화│
# │            ▼                                                         │
# │      [I] 모델  Xception → GAP → Dense(512)                           │
...
#
# ▶ 백본 - 상업 사용 가능한 Xception 계열
#     legacy_xception   기본. Chen 2020 이 쓴 원안 Xception (Chollet 2017).
#                       timm 가중치는 Cadene/pretrained-models.pytorch 포팅본,
#                       BSD-3-Clause -> 상업 사용 가능
#     xception41 / xception65   timm 의 Aligned-Xception (Apache-2.0).
#                       legacy 생성 실패 시 자동 폴백
#     ※ CC BY-NC 계열 가중치는 코드에서 차단합니다
#
# ▶ 실행: python xception_single_640x448_v7.py
...
#
# ▶ 산출물  checkpoints_xception_single_v7/
...
#   logs/xception_single_v7_<ts>.log   실행 로그 전체
# =========================================================================
...
RUN_STATE = LOG_DIR / "xception_single_v7_running.json"
_WORKER_ENV = "BONEAGE_XCEPTION_SINGLE_V7_WORKER"
...
def _spawn_detached():
    ...
    log_path = LOG_DIR / f"xception_single_v7_{ts}.log"
...
#   448/640 = 0.70 인 비정사각 캔버스를 씁니다. Xception 도 완전 합성곱이라
#   비정사각 입력이 그대로 동작합니다 (분류 헤드를 떼고 GAP 를 쓰므로
#   사전학습 해상도 299x299 와 달라도 문제없습니다).
IMG_H, IMG_W = 640, 448
...
# ── 백본 ─────────────────────────────────────────────────────────────
#   [논문] Chen 2020 Table II 에서 Xception 이 최우수 (사전학습 시 8.8 vs
#   InceptionV3 9.2 / ResNet50 9.3 / VGG19 9.3). depthwise separable conv 로
#   자유 파라미터가 적어 과적합에 강하다는 것이 논문의 해석입니다.
BACKBONE     = "legacy_xception"
BACKBONE_ALT = ["xception41", "xception65"]
DROP_PATH    = 0.0          # legacy_xception 은 stochastic depth 미지원.
                            #   xception41/65 로 폴백되면 이 값이 적용됩니다.

#   상업 사용 불가 가중치 차단 목록 (부분 문자열 매칭)
...
# ── 최적화 ───────────────────────────────────────────────────────────
#   유효 배치(=BATCH_SIZE x ACCUM_STEPS)는 v5/v6 과 똑같이 32 로 맞춥니다.
#   VRAM 이 부족하면 아래 프리플라이트가 BATCH_SIZE 를 반으로 줄이고
#   ACCUM_STEPS 를 두 배로 늘려 유효 배치를 보존합니다.
BATCH_SIZE    = 8           # Xception 은 초반 고해상도 단계가 무겁습니다
ACCUM_STEPS   = 4           # 유효 배치 = 32
AUTO_FIT_BATCH = True       # 학습 시작 전 실제 forward/backward 로 VRAM 확인
MIN_BATCH_SIZE = 2          # 여기까지 줄여도 OOM 이면 중단
EPOCHS        = 40          # 코사인 스케줄이 이 값에 맞춰 감깁니다
LR_HEAD       = 3e-4        # [논문] Chen 2020 Phase II 초기 학습률
LR_BACKBONE   = 1e-4        # [추론] 사전학습 특징 보존 -> 헤드보다 낮게
WEIGHT_DECAY  = 1e-4        # [추론] 5e-2 는 ConvNeXt 전용 레시피입니다.
                            #   Xception 은 BN 기반 CNN 이라 1e-4 가 표준입니다.
                            #   v6 와 완전히 같은 레시피로 맞추려면 0.05 로 두세요.
WARMUP_EPOCHS = 3
...
#
#   USE_LDL=False 로 두면 v5와 수치까지 동일한 스칼라 회귀로 되돌아갑니다.
...
LDL_LAMBDA = 0.5    # [논문] 식(12) 트레이드오프 λ. 최적 구간 [0.1, 1] 의 중앙값
...
#   캐시는 '8bit 캔버스 PNG' 이고 전처리 설정이 v5와 완전히 동일하므로
#   v5 캐시를 그대로 재사용합니다 (14,000장 재생성 불필요).
#   설정을 하나라도 바꾸면 PRE_TAG 가 달라져 폴더가 자동 분리됩니다.
CACHE_DIR = BASE_DIR / "cache_convnext_single_v5" / PRE_TAG
#   체크포인트는 헤드 shape 가 다르므로 반드시 분리 (스칼라 240-way 충돌 방지)
CKPT_DIR  = BASE_DIR / ("checkpoints_xception_single_v7_ldl" if USE_LDL
                        else "checkpoints_xception_single_v7_scalar")
...
# ── 화소 정규화 상수 ─────────────────────────────────────────────────
#   [중요] 백본마다 사전학습 시 쓴 mean/std 가 다릅니다.
#     ConvNeXt(ImageNet 레시피) : mean .485/.456/.406, std .229/.224/.225
#     Xception(Inception 레시피): mean .5/.5/.5,        std .5/.5/.5
#   틀린 상수를 쓰면 입력 분포가 사전학습과 어긋나 백본이 제 성능을 못 냅니다.
#   그래서 하드코딩하지 않고 timm 의 pretrained_cfg 에서 읽어옵니다.
def resolve_norm_stats(name, fallback=((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))):
    """백본 이름 -> (mean, std). 조회 실패 시 fallback."""
    try:
...
NORM_MEAN, NORM_STD = resolve_norm_stats(BACKBONE)
#   정규화 공간에서 '픽셀값 0' 에 해당하는 값. 회전 TTA 의 여백을 이걸로
#   채웁니다. 0 을 넣으면 중간 회색이 들어가 TTA 가 손해가 됩니다.
PAD_NORM = [-m / s for m, s in zip(NORM_MEAN, NORM_STD)]
...
print("=" * 72)
log("Xception + LDL 기대값 회귀 골연령 시작 (v7)"
    if USE_LDL else "Xception 단일 스칼라 회귀 골연령 시작 (v7)")
...
log(f"화소정규화     mean {NORM_MEAN} · std {NORM_STD}  [{BACKBONE} pretrained_cfg]")
...
# =========================================================================
# [I] 모델
#     Xception 백본 -> 헤드 -> 성별 임베딩과 결합 -> 240 로짓(또는 스칼라)
#     640x448 입력이면 백본 출력 특징맵은 2048 x 20 x 14 입니다
#     (Xception 은 32배 다운샘플: 640/32=20, 448/32=14).
# =========================================================================
def make_backbone(name=None, pretrained=True, drop_path=None):
    """백본 생성. 실패하면 BACKBONE_ALT 후보로 순차 폴백.

    legacy_xception 은 drop_path_rate 인자를 받지 않습니다. timm 버전에 따라
    조용히 무시되기도 하고 TypeError 를 내기도 해서, drop_path 를 준 시도가
    실패하면 인자 없이 한 번 더 시도합니다 (조용한 실패를 만들지 않기 위해
    어느 경로로 성공했는지 로그에 남깁니다).
    """
    ...
    for cand in [name] + [c for c in BACKBONE_ALT if c != name]:
        if any(d in cand for d in BACKBONE_DENY):
            raise SystemExit(f"[중단] {cand} 는 상업 사용 불가 가중치입니다.")
        kw_list = []
        if drop_path and drop_path > 0:
            kw_list.append({"drop_path_rate": drop_path})
        kw_list.append({})                       # drop_path 없이 재시도
        for kw in kw_list:
            try:
                ...
                log(f"[backbone] {cand} (pretrained={pretrained}, "
                    f"drop_path={kw.get('drop_path_rate', '미지원/미사용')})")
                _m, _s = resolve_norm_stats(cand)
                if [round(v, 4) for v in _m] != [round(v, 4) for v in NORM_MEAN]:
                    log(f"[backbone] [경고] 폴백 백본의 정규화 상수가 다릅니다 "
                        f"{_m} vs 현재 {NORM_MEAN} -> 이 실행은 다시 시작하세요.")
                return m, cand
...
class XceptionRegressor(nn.Module):
    """gap  : GAP -> LayerNorm -> Dense(HEAD_DIM) -> 성별 concat -> Dense(128) -> out
       paper: Conv3x3(256) -> MaxPool3x3 -> Flatten -> 성별 concat -> Dense(128) -> out
              (Chen 2020 Fig.2(b): Conv -> MaxPool -> FC 구조에 대응)
...
def build_param_groups(model, lr_head, lr_backbone, wd):
    ...
    BatchNorm 가중치와 bias(ndim<=1)에 weight decay 를 주면 성능이 떨어집니다.
    base_lr 을 그룹에 심어두고 스케줄러가 배율만 곱합니다.
    """
...
class LDLExpectationLoss(nn.Module):
    ...
    def forward(self, logits, y_months):
        ...
        l_reg = (G * (G.clamp_min(1e-12).log() - logp)).sum(1).mean()   # (식 10)
...
# =========================================================================
def fit_batch_size():
    """학습 시작 전에 실제 forward+backward 를 한 번 돌려 VRAM 을 확인합니다.

    왜 필요한가: Xception 은 초반 320x224x128 단계가 메모리를 크게 먹어서
    8GB 카드에서는 배치 8 이 아슬아슬합니다. 30분 학습한 뒤 OOM 으로
    죽는 것보다 시작 전 10초 안에 알아내는 편이 낫습니다.

    OOM 이면 BATCH_SIZE 를 반으로 줄이고 ACCUM_STEPS 를 두 배로 늘립니다.
    유효 배치(=곱)가 보존되므로 그래디언트 수학은 그대로입니다.
    바뀌는 것은 BatchNorm 이 한 번에 보는 표본 수뿐이며, 이는 로그에 남깁니다.
    """
    global BATCH_SIZE, ACCUM_STEPS
    if not (AUTO_FIT_BATCH and torch.cuda.is_available()):
        return
    log(f"[프리플라이트] VRAM 확인 (배치 {BATCH_SIZE} 부터)")
    while True:
        probe = crit_probe = opt = None
        try:
            probe = build_model(ARCH, pretrained=False, verbose=False)
            opt = torch.optim.AdamW(probe.parameters(), lr=1e-5)
            sc = torch.amp.GradScaler("cuda", enabled=USE_AMP)
            x = torch.randn(BATCH_SIZE, 3, IMG_H, IMG_W, device=device
                            ).to(memory_format=torch.channels_last)
            g = torch.zeros(BATCH_SIZE, 1, device=device)
            y = torch.full((BATCH_SIZE,), 120.0, device=device)
            crit_probe = make_criterion()
            with torch.amp.autocast("cuda", enabled=USE_AMP):
                out = probe(x, g)
            loss = (crit_probe(out.float(), y)[0] if USE_LDL
                    else crit_probe(out, (y - AGE_MEAN) / AGE_STD))
            sc.scale(loss).backward(); sc.step(opt); sc.update()
            peak = torch.cuda.max_memory_allocated() / 1024 ** 3
            total = torch.cuda.get_device_properties(0).total_memory / 1024 ** 3
            log(f"[프리플라이트] 통과 · 배치 {BATCH_SIZE} x 누적 {ACCUM_STEPS} "
                f"(유효 {BATCH_SIZE*ACCUM_STEPS}) · 피크 {peak:.2f}/{total:.1f}GB")
            if BATCH_SIZE < 8:
                log(f"[프리플라이트] [주의] BatchNorm 이 한 번에 보는 표본이 "
                    f"{BATCH_SIZE}개입니다. 통계가 불안정하면 IMG 해상도를 낮추거나 "
                    f"USE_GRAD_CKPT=True 로 배치를 되돌리세요.")
            break
        except torch.cuda.OutOfMemoryError:
            if BATCH_SIZE <= MIN_BATCH_SIZE:
                raise SystemExit(
                    f"[중단] 배치 {MIN_BATCH_SIZE} 에서도 OOM 입니다.\n"
                    f"       USE_GRAD_CKPT=True 로 켜거나 IMG_H/IMG_W 를 낮추세요.")
            BATCH_SIZE //= 2; ACCUM_STEPS *= 2
            log(f"[프리플라이트] OOM -> 배치 {BATCH_SIZE} · 누적 {ACCUM_STEPS} 로 재시도")
        finally:
            del probe, crit_probe, opt
        ...
        for epoch in range(start_epoch, EPOCHS + 1):
            ...
            for it, (x, g, yn, ym) in enumerate(train_loader):
                ...
                with torch.amp.autocast("cuda", enabled=USE_AMP):
                    pred = model(x, g)
                if USE_LDL:
                    loss_all, l_mae, l_reg = criterion(pred.float(), ym_d)
...
def save_learning_curve():
    ...
    try:
        ...
        ax[0].set_title(f"Xception {'LDL' if EM_LDL else 'scalar'} "
...
class GradCAM:
    """Xception 마지막 특징맵 기준 Grad-CAM (기대값 출력)."""
    ...
    @staticmethod
    def _pick(backbone):
        """마지막 공간 해상도를 가진 모듈을 찾습니다.
           legacy_xception : act4 (conv4-bn4-act4 가 마지막 블록)
           xception41/65   : blocks (XceptionAligned)
        """
        mods = dict(backbone.named_modules())
        for n in ("act4", "bn4", "conv4", "blocks", "stages.3", "norm_pre"):
            if n in mods:
                return mods[n], n
