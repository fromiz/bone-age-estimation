...
#
#   ▶ v6 (현재 버전) = v5 전처리·R1 유지 + R2/집계를 논문 방식으로 복원
#     [유지] 전처리 5단계: ①정규화 ②Top-Hat ③CLAHE ④비율유지 리사이즈 ⑤배경색 패딩
#     [유지] R1(수근골): 고정 밴드 안 CAM + 박스 확대 -> QC 에서 맞춘 크기 그대로
#     [유지] 레터박스 좌표 역변환(canvas_box_to_src), ROI 크롭 비율유지+패딩
#     [복원] R2(중수골): ★ 논문 §III-A 방식
#            R1 을 랜덤값으로 지운 E 로 분류모델을 '다시 학습' -> 그 CAM 이 R2.
#            밴드 제약 없음 / 박스 확대 없음 / tau=50 / 최대 연결성분(Eq.4)
#     [복원] 3채널 집계: 논문 Table V (H+R1+R2 = 4.3 / H+R1+E = 4.3)
#            서로 다른 국소 패치를 입력 '채널'에 하나씩 (공식 main_aggregation.py)
#     [유지] Phase II: Xception + 성별(±1)→FC32 + 240-way softmax 기대값 회귀
#            손실 = l_MAE + λ·D_KL(G‖p), δ=15, λ=0.5 (논문 Eq.9~12)
#     [분리] cache_attention_v6/<PRE_TAG> / checkpoints_attention_v6 / logs/*_v6_*
#
#   ▶ v5 대비 학습 비용: 분류모델이 1개 -> 2개 (cls-r1, cls-r2) 로 늘어납니다.
#
...
RUN_STATE = LOG_DIR / "boneage_attention_v6_running.json"
_WORKER_ENV = "BONEAGE_ATTENTION_V6_WORKER"
...
def _spawn_detached():
    ...
    log_path = LOG_DIR / f"boneage_attention_v6_{ts}.log"
...
CACHE_DIR = BASE_DIR / "cache_attention_v6" / PRE_TAG
CKPT_DIR  = BASE_DIR / "checkpoints_attention_v6"
...
def build_qc_sheet(n=N_QC, tag="qc_roi"):
    ...
    cols = ["raw (전처리 전)", "전처리+R1밴드+박스", "r1 crop", "E + R2박스", "r2 crop"]
    ...
    log(f"QC 시트 저장: {out}")
    log("  1열=전처리 전 / 2열=전처리+R1밴드(회색)+R1박스(빨강) / 4열=E+R2박스(파랑)")
    for mm in (m1, m2):
        if mm is not None:
            del mm
...
if EVAL_ONLY:
    ...
    if AUTO_RESUME and LAST_CKPT.exists():
        ...
        if bad:
            ...
            log("  * 처음부터 다시 하려면 checkpoints_attention_v6 의 last.pt 삭제 후 실행")
    else:
