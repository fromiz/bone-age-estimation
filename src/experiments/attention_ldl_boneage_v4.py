...
#     2) ★ R1 / R2 를 '각자의 고정 밴드' 안에서 독립적으로 추출
#        - R2 가 R1 에 종속되지 않음 (겹쳐도 무방)
#        - 분류모델은 1개만 학습 (CAM 공급용)
#     3) ★ 크기 맞추기는 '자른 뒤 검은 여백 패딩' (PAD_TO_SQUARE)
#        - 박스를 늘려 정사각을 만들지 않으므로 관심 밖 조직이 안 들어옴
#        - 뼈 형태 종횡비 왜곡 0
#     4) 조기 종료 + 중단 시 자동 이어서 학습 (Phase I / II 모두)
#     5) 저장 위치/로그를 v4 전용으로 분리 (이전 버전과 충돌 없음)
#        cache_attention_v4 / checkpoints_attention_v4 / logs/boneage_attention_v4_*
#
#   ▶ 밴드 튜닝 (별도 preview 파일 불필요)
#       python attention_ldl_boneage_v4.py --qc-only
#       -> ROI 생성 + QC 시트만 만들고 종료. checkpoints_attention_v4/qc_roi.png 확인
...
RUN_STATE = LOG_DIR / "boneage_attention_v4_running.json"
_WORKER_ENV = "BONEAGE_ATTENTION_V4_WORKER"
...
def _spawn_detached():
    ...
    log_path = LOG_DIR / f"boneage_attention_v4_{ts}.log"
...
# -- ★ 크기 맞추기: 박스를 늘리지 말고 '자른 뒤 패딩' ------------------------
#   True  : 필요한 만큼만 자르고 검은 여백으로 정사각 -> 왜곡 0, 조직 유입 0
#   False : 정사각 강제 리사이즈 -> 뼈 형태가 늘어남
PAD_TO_SQUARE = True
...
CACHE_DIR = BASE_DIR / "cache_attention_v4" / f"{CLS_SIZE_REGION}"
CKPT_DIR  = BASE_DIR / "checkpoints_attention_v4"
...
def crop_from_source(src_gray, box_in_ref, ref_w, ref_h, size=REG_SIZE,
                     pad_square=None):
    """ref(=CAM 좌표계, 보통 560) 의 bbox 를 src(손 크롭 원본 해상도)로 역스케일해 크롭.
       ★ v4: 박스를 늘려 정사각을 만들지 않고, 필요한 만큼만 자른 뒤
              검은 여백(레터박스)으로 정사각을 맞춥니다.
              -> 종횡비 왜곡 0 + 관심 밖 조직이 들어오지 않음."""
    ...
    if patch.size == 0:
        patch = src_gray
    if pad_square:
        return to_square(patch, size, pad=True)      # 자른 뒤 여백 패딩
...
def localize_roi(model, records):
    """★ v4: R1 과 R2 를 '각자의 고정 밴드' 안에서 독립적으로 찾는다.
       - 한 장당 CAM 을 1회만 계산하고, 밴드만 바꿔 두 영역을 각각 추출
       - R1 이 R2 에 영향을 주지 않음 (서로 독립, 겹쳐도 무방)
       - 크롭은 손 크롭 원본 해상도에서 -> 자른 뒤 패딩으로 정사각(왜곡 0)
       - E(erased) 는 R1 박스를 랜덤값으로 덮어 생성 (채널 옵션용)"""
    model.eval()
    ...
    for sp in SPLITS:
        ...
        for i, (_, r) in enumerate(df.iterrows(), 1):
            ...
            ref = imread_kr(CACHE_DIR / "hand560" / sp / f"{r['id']}.png",
...
def build_qc_sheet(n=N_QC, tag="qc_roi"):
    """밴드/박스 튜닝용 QC 시트. 학습된 cls 모델을 로드해 CAM 을 다시 계산합니다."""
    if not len(val_df):
        return
    ...
    cols = ["hand+band+box", "r1 crop", "r2 crop", "erased"]
    ...
    for rr, (_, r) in enumerate(samp.iterrows()):
        ...
        if ref is not None and m is not None:
            ...
            cam_r = cv2.resize(cam.astype(np.float32), (CLS_SIZE_REGION, CLS_SIZE_REGION))
            ...
            for kind in ("r1", "r2"):
                ...
                boxes[kind] = b
        if ref is not None:
            axes[rr, 0].imshow(_overlay_bands(ref, cam_norm, boxes))
            ttl = f"{r['id']}"
            if boxes:
                ttl += (f"  R1 {boxes['r1'][2]-boxes['r1'][0]}x{boxes['r1'][3]-boxes['r1'][1]}"
                        f"  R2 {boxes['r2'][2]-boxes['r2'][0]}x{boxes['r2'][3]-boxes['r2'][1]}")
            axes[rr, 0].set_title(ttl, fontsize=7)
        for cc, k in enumerate(["r1", "r2", "erased"], start=1):
...
if EVAL_ONLY:
    ...
    if AUTO_RESUME and LAST_CKPT.exists():
        ...
        if bad:
            ...
            log("  * 처음부터 다시 하려면 checkpoints_attention_v4 의 last.pt 삭제 후 실행")
    else:
...
def make_rois_from_hand(hand_gray, channels=AGG_CHANNELS):
    """손 크롭 -> {kind: 560x560}. 학습과 동일 규칙(독립 밴드 + 자른 뒤 패딩)."""
    out = {}
    ref = to_square(hand_gray, CLS_SIZE_REGION)
    out["hand"] = ref
    need = [k for k in ("r1", "r2") if k in channels] or (["r1"] if "erased" in channels else [])
    if need or "erased" in channels:
        m = _load_cls(CLS_R1_BEST)
        x = torch.from_numpy(np.stack([ref] * 3, 0)).float().div(255.).unsqueeze(0).to(device)
        cam, _ = compute_cam(m, x)
        boxes = {}
        for kind in set(need + (["r1"] if "erased" in channels else [])):
            band = band_abs(kind, CLS_SIZE_REGION, CLS_SIZE_REGION)
            b, _ok = cam_to_bbox(cam, TAU_R1 if kind == "r1" else TAU_R2,
                                 CLS_SIZE_REGION, CLS_SIZE_REGION,
                                 box_cfg=BOX_CFG.get(kind), region=band,
            ...
            boxes[kind] = b
            if kind in channels:
                out[kind] = crop_from_source(hand_gray, b, CLS_SIZE_REGION, CLS_SIZE_REGION)
