# -*- coding: utf-8 -*-
# =========================================================================
# 손 X-ray 골연령 추정 - ConvNeXt-Tiny + LDL + FiLM + 좌우/회전 교정  [v12]
#
# 베이스: v11 (FiLM 성별조건화). 증강 오류 2건을 바로잡습니다.
#
# ── v11 대비 변경점 (3가지) ───────────────────────────────────────────
#   1. 회전 증강  12° -> 5°   [오류 수정]
...
# │            ▼                                                         │
# │      [F] 캐시  cache_convnext_single_v12/<PRE_TAG>/  ← v12 전용      │
# │            ▼                                                         │
...
#
# ▶ 실행: python convnexttiny_single_768x512_v12.py
...
#
# ▶ 산출물  checkpoints_convnext_single_v12_ldl/  (USE_LDL=False 면 _scalar)
...
#   logs/convnext_single_v12_<ts>.log   실행 로그 전체
# =========================================================================
...
RUN_STATE = LOG_DIR / "convnext_single_v12_running.json"
_WORKER_ENV = "BONEAGE_CONVNEXT_SINGLE_V12_WORKER"
...
def _spawn_detached():
    ...
    log_path = LOG_DIR / f"convnext_single_v12_{ts}.log"
...
HAND_CROP_DIR = BASE_DIR / "crop_data_yolox_s"        # training / validation / test 하위폴더
CSV_DIR       = BASE_DIR / "crop_data_yolox_s"   # training.csv / validation.csv / test.csv
...
CACHE_DIR = BASE_DIR / "cache_convnext_single_v12" / PRE_TAG
#   체크포인트는 헤드 shape 가 다르므로 반드시 분리 (스칼라 240-way 충돌 방지)
CKPT_DIR  = BASE_DIR / ("checkpoints_convnext_single_v12_ldl" if USE_LDL
                        else "checkpoints_convnext_single_v12_scalar")
...
print("=" * 72)
log("ConvNeXt-Tiny + LDL 기대값 회귀 골연령 시작 [v12]"
    if USE_LDL else "ConvNeXt-Tiny 스칼라 회귀 골연령 시작 [v12]")
...
log(f"[v12] 회전 {AUG_ROT_DEG}° (v11 12°) · 스케일 {AUG_SCALE} · 좌우반전 p={AUG_HFLIP_P} (신규)")
log(f"[v12] 평가 리포트: 성별 분해 · 성별x연령 교차 · 성별 플립 · 분포 형태(왜도)")
...
def dist_shape_stats(model, df, split, bs=16):
    ...
    with torch.no_grad():
        for x, g, _, ym in loader:
            ...
            MU.append(mu.cpu()); T.append(ym.float())
    sd = torch.cat(SD).numpy(); sk = torch.cat(SK).numpy()
    mu = torch.cat(MU).numpy(); t = torch.cat(T).numpy()
    err = mu - t
