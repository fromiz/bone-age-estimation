...
#          (같은 split / 같은 EXCLUDE 조건 / 같은 512 레터박스 / 같은 ConvNeXt V1 계열)
#
...
#       ※ convnextv2_* 는 가중치가 CC BY-NC 4.0 -> 코드에서 assert 로 차단
#
#   ▶ v8/v10 대비 하이퍼파라미터 개선
#       AdamW(norm/bias wd=0) · 차등 LR · warmup+cosine · drop_path · EMA
#       · grad accumulation(유효 배치 32) · Huber
#
#   ▶ 실행: python convnexttiny_single_letterbox512.py
...
#       --rebuild-cache  512 캔버스 캐시를 강제로 재생성
#       --qc-only        QC 시트만 만들고 종료 (전처리 통과 검증용)
#       --fresh          last.pt 를 무시하고 처음부터 학습
#
#   ▶ 산출물
#       logs/convnext_single_<타임스탬프>.log      실행 로그 전체
#       checkpoints_convnext_single/env_<ts>.json  실행 환경 스냅샷
#       checkpoints_convnext_single/best.pt        최고 검증 MAE 가중치
#       checkpoints_convnext_single/last.pt        매 에폭 저장 (재개용)
#       checkpoints_convnext_single/history.json   에폭별 지표
#       checkpoints_convnext_single/results.txt|json  최종 성적표
#       checkpoints_convnext_single/*.png          QC/곡선/산점도/GradCAM
# =========================================================================
...
RUN_STATE = LOG_DIR / "convnext_single_running.json"
_WORKER_ENV = "BONEAGE_CONVNEXT_SINGLE_WORKER"
...
def _spawn_detached():
    ...
    log_path = LOG_DIR / f"convnext_single_{ts}.log"
...
EPOCHS       = 100
LR_HEAD      = 1e-4
LR_BACKBONE  = 4e-5         # 사전학습 특징 보존 -> 헤드의 1/10
...
# ── 조기 종료 / 실행 ─────────────────────────────────────────────────
EARLY_STOP_PATIENCE = 12    # val MAE 가 이 에폭 수만큼 개선 없으면 중단 (<=0 이면 끔)
...
CACHE_DIR = BASE_DIR / "cache_convnext_single_1" / PRE_TAG
CKPT_DIR  = BASE_DIR / "checkpoints_convnext_single_1"
...
print("=" * 72)
log("ConvNeXt-Tiny 단일 회귀 골연령 - 전처리 없음 / 레터박스 512 시작")
...
results, lines = {}, []
lines.append("=" * 60)
lines.append(" ConvNeXt-Tiny 단일 회귀 / 전처리 없음 / 레터박스 512")
...
for sp, df in [("train", train_df), ("val", val_df)]:
    p, t, _ = predict_split(eval_model, df, sp)
...
def save_learning_curve():
    ...
    try:
        ...
        ax[0].set_title("ConvNeXt-Tiny single / no preprocessing")
