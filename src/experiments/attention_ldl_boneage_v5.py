...
#
#   ▶ v5 (현재 버전) 핵심
#     1) 손(Hand)은 이미 크롭된 폴더(yolo_crop)를 그대로 사용
#     2) ★ 전처리 5단계 추가 (골연령_Xception_TopHat_CLAHE 노트북 이식)
#          ①밝기정규화(백분위) → ②Top-Hat → ③CLAHE → ④비율유지 리사이즈 → ⑤배경색 패딩
#          - Top-Hat 은 '원본 해상도'에서 수행 (커널이 뼈 굵기보다 커야 의미 있음)
#          - 전처리 설정이 캐시 폴더명(PRE_TAG)에 들어가 실험이 섞이지 않음
#     3) ★ 레터박스 패딩 좌표 보정 (canvas_box_to_src)
#          캔버스 좌표 != 원본 좌표 단순비례 -> scale + pad 오프셋을 역산.
#          (역변환 없이 단순비례하면 실측 80px 어긋남)
#     4) R1 / R2 를 각자의 고정 밴드에서 독립 추출 (서로 종속 없음)
#     5) ROI 크롭도 비율유지 리사이즈 + 배경색 패딩 -> 종횡비 왜곡 0
#     6) 조기 종료 + 중단 시 자동 이어서 학습
#     7) 저장 위치/로그를 v5 전용으로 분리
#        cache_attention_v5/<PRE_TAG> / checkpoints_attention_v5 / logs/boneage_attention_v5_*
#
#   ▶ 밴드 튜닝 (별도 preview 파일 불필요)
#       python attention_ldl_boneage_v4.py --qc-only
#       -> ROI 생성 + QC 시트만 만들고 종료. checkpoints_attention_v5/qc_roi.png 확인
#          (회색=밴드, 빨강=R1 박스, 파랑=R2 박스, CAM 히트맵 오버레이)
#       -> R1_BAND / R2_BAND / BOX_CFG 수정 후 --qc-only --rebuild-roi 로 반복
#
...
RUN_STATE = LOG_DIR / "boneage_attention_v5_running.json"
_WORKER_ENV = "BONEAGE_ATTENTION_V5_WORKER"
...
def _spawn_detached():
    ...
    log_path = LOG_DIR / f"boneage_attention_v5_{ts}.log"
...
PRE_TAG   = (f"{REG_SIZE}_n{_NORM_TAG}_th{_TH_TAG}"
             f"_c{CLAHE_CLIP:g}t{CLAHE_TILE}_{_SC_TAG}_pad{PAD_MODE}")

# =========================================================================
# ★ [v4] R1 / R2 를 각각 '독립 밴드' 안에서 찾습니다
#
#   이전 문제: R2 를 'R1 박스 위쪽'으로 정의 -> R1 에 종속. 또 min_w/min_h 가
#              커서 박스가 손 전체로 부풀어 R1 == hand 가 되어버림.
#   v4 방식  : R1 과 R2 각각 '이미지 비율 기준 고정 밴드'를 지정하고, 그 밴드
#              안에서만 CAM 최대 영역을 찾는다. 서로 독립이며 겹쳐도 무방.
#
#   밴드 좌표 = 손 크롭 이미지(560x560) 비율 기준 (0.0~1.0)
#     R1 = 수근골(손목)      : 아래쪽        <- QC 그림의 빨강 박스
#     R2 = 중수골+근위지골   : 위쪽 넓게      <- QC 그림의 파랑 박스
#   ※ 손목이 이미지 아래쪽에 오도록 크롭돼 있다는 전제입니다.
# =========================================================================
R1_BAND = {"x0": 0.12, "y0": 0.52, "x1": 0.88, "y1": 0.98}   # 수근골 밴드
R2_BAND = {"x0": 0.06, "y0": 0.10, "x1": 0.94, "y1": 0.62}   # 중수골 밴드
BAND_CFG = {"r1": R1_BAND, "r2": R2_BAND}
CLAMP_TO_BAND = True        # 확대된 박스가 밴드 밖으로 못 나가게 고정

# -- 박스 확대 파라미터 -------------------------------------------------------
#   [v4 조정] min_* 는 '강제 확대'가 아니라 너무 작아질 때만 걸리는 안전장치.
...
PAD_FRAC_R2_X      = 0.08
PAD_FRAC_R2_Y      = 0.10
MIN_W_FRAC_R2      = 0.62   # (QC 파랑 박스 기준)
MIN_H_FRAC_R2      = 0.42
...
# -- 크기 맞추기 -------------------------------------------------------------
#   ROI 크롭도 전처리와 동일하게 '비율 유지 리사이즈 + 배경색 패딩'(fit_canvas)
#   으로 정사각을 맞춥니다 -> 종횡비 왜곡 0, 관심 밖 조직 유입 0
PAD_TO_SQUARE = True
...
CACHE_DIR = BASE_DIR / "cache_attention_v5" / PRE_TAG
CKPT_DIR  = BASE_DIR / "checkpoints_attention_v5"
...
def enhance(gray, norm_mode=None, norm_lo=None, norm_hi=None,
            th_mode=None, th_kfrac=None, th_ksize=None, th_shape=None, th_weight=None,
            clip=None, tile=None, return_k=False):
    """①정규화 → ②Top-Hat → ③CLAHE 까지만 (해상도 변경 없음).
       ★ 원본 해상도에서 1회만 수행하고, 캔버스/ROI 크롭에 재사용합니다."""
    g = normalize_intensity(gray, norm_mode, norm_lo, norm_hi)
    g, k = apply_tophat(g, th_mode, th_kfrac, th_ksize, th_shape, th_weight, return_k=True)
    g = apply_clahe(g, clip, tile)
    return (g, k) if return_k else g


def preprocess(gray, size=None, return_info=False, **kw):
    """전체 5단계: enhance -> 배율 -> 리사이즈+패딩."""
    size = REG_SIZE if size is None else size
    g, k = enhance(gray, return_k=True, **kw)
...
@torch.no_grad()
def localize_roi(model, records):
    """★ v5: 전처리된 캔버스에서 CAM -> 밴드별 박스 -> 원본 좌표 역변환 ->
       전처리된 '원본 해상도'에서 고해상도 재크롭 -> 비율유지+패딩으로 560.
       R1 / R2 는 각자의 고정 밴드 안에서 독립적으로 찾습니다."""
    model.eval()
    taus = {"r1": TAU_R1, "r2": TAU_R2}
    for sp in SPLITS:
        df = SPLIT_DFS[sp]
        if not len(df):
            continue
        stat = {"r1": [0, 0], "r2": [0, 0]}      # [CAM기반, 밴드폴백]
        miss = 0
        for i, (_, r) in enumerate(df.iterrows(), 1):
            dsts  = {k: CACHE_DIR / k / sp / f"{r['id']}.png" for k in ("r1", "r2")}
            dst_e = CACHE_DIR / "erased" / sp / f"{r['id']}.png"
            if (all(d.exists() for d in dsts.values()) and dst_e.exists()
                    and not REBUILD_ROI):
                continue
            ...
            # 전처리를 원본 해상도에서 1회 수행하고 캔버스/ROI 크롭에 재사용
            enh_full = enhance(raw)                       # ①정규화 ②TopHat ③CLAHE
            ref, info = fit_canvas(enh_full, REG_SIZE)    # ④배율 ⑤패딩 -> 캔버스
            ...
            boxes = {}
            for kind in ("r1", "r2"):
                band = band_abs(kind, REG_SIZE, REG_SIZE)
                box, ok = cam_to_bbox(cam, taus[kind], REG_SIZE, REG_SIZE,
                                      box_cfg=BOX_CFG.get(kind), region=band,
                                      clamp_region=band if CLAMP_TO_BAND else None,
                                      fallback_box=band)
                boxes[kind] = box
                stat[kind][0 if ok else 1] += 1
                imwrite_kr(dsts[kind], crop_roi(enh_full, box, info, REG_SIZE))
                ...
                records.append({"split": sp, "id": r["id"], "kind": kind,
                                ...
                                "ref_size": REG_SIZE, "found": int(ok)})

            # E: R1 박스를 랜덤값으로 덮은 전체 손 캔버스
            e = ref.copy()
            ...
            if i % 1000 == 0:
                log(f"  [roi] {sp} {i}/{len(df)}")
        log(f"[roi:{sp}] R1 CAM {stat['r1'][0]} / 폴백 {stat['r1'][1]} | "
            f"R2 CAM {stat['r2'][0]} / 폴백 {stat['r2'][1]} | 파일없음 {miss}")
...
if roi_cache_valid():
    ...
    # --- R1(수근골) / R2(중수골) 를 각자 밴드에서 독립 추출 + E 생성 ----------
    #     분류모델은 1개만 학습합니다 (CAM 공급용). R2 는 R1 에 종속되지 않고
    #     자기 밴드 안에서 따로 찾습니다.
    log("### Phase I : R1(수근골) / R2(중수골) 독립 국소화 + Erased ###")
    log(f"  R1 밴드 {R1_BAND} | R2 밴드 {R2_BAND} | clamp {CLAMP_TO_BAND}")
    log(f"  크기 맞추기: {'자른 뒤 패딩(왜곡 0)' if PAD_TO_SQUARE else '정사각 강제 리사이즈'}")
    m_cam = train_cam_classifier("cls-r1", CLS_R1_BEST, CLS_R1_LAST, "gmp", "hand560")
    localize_roi(m_cam, bbox_records)
    del m_cam; torch.cuda.empty_cache()

    if bbox_records:
        pd.DataFrame(bbox_records).to_csv(BBOX_CSV, index=False, encoding="utf-8-sig")
        log(f"bbox 로그 저장: {BBOX_CSV}")
        # 박스 크기 요약 -> 밴드/파라미터 튜닝 판단용
        try:
            ...
            for k in ("r1", "r2"):
                ...
                if len(sub):
                    log(f"  [{k}] 박스 폭 중앙값 {sub['w'].median():.0f}px "
                        f"({sub['w'].median()/CLS_SIZE_REGION:.0%}) | "
                        f"높이 중앙값 {sub['h'].median():.0f}px "
                        f"({sub['h'].median()/CLS_SIZE_REGION:.0%})")
        except Exception:
            pass
...
def build_qc_sheet(n=N_QC, tag="qc_roi"):
    """밴드/박스/전처리 튜닝용 QC 시트.
       열: [원본] [전처리 후 캔버스+밴드+박스] [r1] [r2] [erased]"""
    if not len(val_df):
        return
    samp = val_df.sample(min(n, len(val_df)), random_state=SEED)
    m = None
    if CLS_R1_BEST.exists():
        try:
            ck = torch_load(CLS_R1_BEST, map_location=device)
            m = CAMClassifier(pool="gmp", pretrained=False).to(device)
            m.load_state_dict(ck["model"]); m.eval()
        except Exception as e:
            log(f"[QC] 모델 로드 실패({e}) - 크롭 결과만 표시")

    cols = ["raw (전처리 전)", "전처리+밴드+박스", "r1 crop", "r2 crop", "erased"]
    fig, axes = plt.subplots(len(samp), len(cols),
                             figsize=(2.7 * len(cols), 2.7 * len(samp)))
    axes = np.atleast_2d(axes)
    for rr, (_, r) in enumerate(samp.iterrows()):
        raw = imread_kr(r["hand_path"], cv2.IMREAD_GRAYSCALE)
        ref = imread_kr(CACHE_DIR / "hand560" / "val" / f"{r['id']}.png",
        ...
        if ref is not None and m is not None:
            with torch.no_grad():
                x = torch.from_numpy(np.stack([ref] * 3, 0)).float().div(255.).unsqueeze(0).to(device)
                cam, _ = compute_cam(m, x)
            cam_r = cv2.resize(cam.astype(np.float32), (REG_SIZE, REG_SIZE))
            cam_norm = cam_r / max(float(cam_r.max()), 1e-8)
            for kind in ("r1", "r2"):
                band = band_abs(kind, REG_SIZE, REG_SIZE)
                b, _ok = cam_to_bbox(cam, TAU_R1 if kind == "r1" else TAU_R2,
                                     REG_SIZE, REG_SIZE,
                                     box_cfg=BOX_CFG.get(kind), region=band,
                                     clamp_region=band if CLAMP_TO_BAND else None,
                                     fallback_box=band)
                boxes[kind] = b
        if ref is not None:
            axes[rr, 1].imshow(_overlay_bands(ref, cam_norm, boxes))
            ttl = "전처리 후"
            if boxes:
                ttl = (f"R1 {boxes['r1'][2]-boxes['r1'][0]}x{boxes['r1'][3]-boxes['r1'][1]}"
                       f" R2 {boxes['r2'][2]-boxes['r2'][0]}x{boxes['r2'][3]-boxes['r2'][1]}")
            axes[rr, 1].set_title(ttl, fontsize=7)
        for cc, k in enumerate(["r1", "r2", "erased"], start=2):
            im = imread_kr(CACHE_DIR / k / "val" / f"{r['id']}.png", cv2.IMREAD_GRAYSCALE)
            if im is not None:
                axes[rr, cc].imshow(im, cmap="gray")
            axes[rr, cc].set_title(cols[cc], fontsize=7)
    ...
    log(f"QC 시트 저장: {out}")
    log("  1열=전처리 전 원본 / 2열=정규화+TopHat+CLAHE+패딩 결과")
    log("  회색=밴드(탐색 허용) / 빨강=R1 박스 / 파랑=R2 박스")
    if m is not None:
...
if EVAL_ONLY:
    ...
    if AUTO_RESUME and LAST_CKPT.exists():
        ...
        if bad:
            ...
            log("  * 처음부터 다시 하려면 checkpoints_attention_v5 의 last.pt 삭제 후 실행")
    else:
...
def make_rois_from_hand(hand_gray, channels=AGG_CHANNELS):
    """손 크롭(원본 해상도) -> {kind: 560x560}. 학습과 완전히 동일한 규칙:
       전처리(정규화/TopHat/CLAHE) -> 캔버스 -> CAM -> 밴드별 박스
       -> 원본 좌표 역변환 -> 고해상도 재크롭 -> 비율유지+패딩."""
    out = {}
    enh_full = enhance(hand_gray)
    ref, info = fit_canvas(enh_full, REG_SIZE)
    out["hand"] = ref
    need = set(k for k in ("r1", "r2") if k in channels)
    if "erased" in channels:
        need.add("r1")
    if need:
        m = _load_cls(CLS_R1_BEST)
        x = torch.from_numpy(np.stack([ref] * 3, 0)).float().div(255.).unsqueeze(0).to(device)
        cam, _ = compute_cam(m, x)
        boxes = {}
        for kind in need:
            ...
            boxes[kind] = b
            if kind in channels:
                out[kind] = crop_roi(enh_full, b, info, REG_SIZE)
        if "erased" in channels and "r1" in boxes:
            e = ref.copy()
            x0, y0, x1, y1 = boxes["r1"]
            e[y0:y1, x0:x1] = np.random.randint(0, 256, (y1 - y0, x1 - x0), dtype=np.uint8)
            out["erased"] = e
