# xception_512_target_norm_cosine.py
# Xception 512x512 + weak augmentation + target normalization + cosine LR scheduler
# 실행: python .\xception_512_target_norm_cosine.py
...
SEED = 42
EXPERIMENT_NAME = "xception_weak_aug_512_target_norm_cosine"
...
TRAIN_CSV_PATH = DATA_ROOT / "boneage-training-dataset" / "train.csv"
TRAIN_IMAGE_DIR = DATA_ROOT / "boneage-training-dataset" / "boneage-training-dataset"

VAL_CSV_PATH = DATA_ROOT / "boneage-validation-dataset" / "Validation Dataset.csv"
VAL_IMAGE_DIR = DATA_ROOT / "boneage-validation-dataset" / "boneage-validation-dataset"

TEST_CSV_PATH = DATA_ROOT / "Bone Age Test Set" / "Bone age ground truth.csv"
TEST_IMAGE_DIR = DATA_ROOT / "Bone Age Test Set" / "Test Set Images"
...
if RUN_TEST_AFTER_TRAINING:
    test_df = normalize_boneage_dataframe(pd.read_csv(TEST_CSV_PATH))
else:
    test_df = None
...
# timm legacy_xception pretrained 설정 확인 결과 mean/std = 0.5 유지
...
def train_one_epoch(
    ...
    for batch in data_loader:
        ...
        total_absolute_error_month += (
            torch.abs(predictions_month - targets_month)
            .sum()
            .item()
        )
...
def cosine_lr_lambda(current_epoch: int) -> float:
    """
    backbone/head의 초기 LR 비율을 유지하면서 전체 LR을 cosine으로 감소시킨다.
    epoch 0에서는 1.0, 마지막에는 min_factor 근처.
    """
    min_factor = 0.01
    progress = min(current_epoch, MAX_EPOCHS) / MAX_EPOCHS
...
for epoch in range(1, MAX_EPOCHS + 1):
    ...
    # Cosine scheduler는 validation metric 없이 매 epoch 1번 step
    scheduler.step()
