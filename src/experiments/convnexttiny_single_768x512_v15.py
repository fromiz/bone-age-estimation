# -*- coding: utf-8 -*-
# =========================================================================
# 손 X-ray 골연령 추정 - ConvNeXt-Tiny + LDL + FiLM + 마스크 전처리  [v15]
#
...
#   -> crop_seg_aligned/{train,validation,test}/{images,masks} 생성
#
# ── v11 대비 변경점 (3가지, 전부 '입력 데이터 정제' 라는 한 축) ────────
#   1. 데이터 소스   crop_data_yolox_s -> crop_seg_aligned
#      v11~v14 는 정렬되지 않은 옛 크롭을 썼습니다. 새 데이터셋은 마스크 기반
#      PCA 정렬이 적용돼 있고 잔차 중앙이 0.0도입니다.
#   2. 마스크 배경 제거
#      QC 시트에서 크롭 안에 판독 마커가 그대로 보입니다("L JMN", "L R", "BG").
#      기관·기사마다 다른 표식이라 모델이 뼈 대신 마커를 보는 지름길을 배울 수
#      있고, 그 지름길은 RSNA 내부 평가에서는 드러나지 않고 배포처에서만
#      무너집니다. 마스크 밖은 프레임의 약 60% 이고 골연령 정보는 0 입니다.
#   3. 마스크 내부 p1p99
#      기존 g>0 조건은 '배경이 정확히 0' 을 가정하는데 실제 X-ray 배경은
#      산란선 때문에 어중간한 회색입니다. 마스크로 계산하면 손 조직만 남습니다.
#
#   + 회전 증강 12도 -> 5도 (입력이 이미 정렬됨. 0 으로 두지는 않음)
#   + 마스크 열화 증강 (배포처 세그멘테이션 품질 저하 대비)
#
# ── 참고 성적 ─────────────────────────────────────────────────────────
#   v11  test MAE 4.05 RMSE 5.10 CI95 [3.64, 4.51] | val 5.78 / 7.99
#   * test N=197 의 CI 폭은 ±0.45 입니다. 0.5 개월 미만 차이는 판단 불가.
#
# ── v10 실험 결론 (원복 근거) ─────────────────────────────────────────
...
# │            ▼                                                         │
# │      [F] 캐시  cache_convnext_single_v15/<PRE_TAG>/  ← v15 전용      │
# │            ▼                                                         │
...
#
# ▶ 실행: python convnexttiny_single_768x512_v15.py
...
#
# ▶ 산출물  checkpoints_convnext_single_v15_ldl/  (USE_LDL=False 면 _scalar)
...
#   logs/convnext_single_v15_<ts>.log   실행 로그 전체
# =========================================================================
...
RUN_STATE = LOG_DIR / "convnext_single_v15_running.json"
_WORKER_ENV = "BONEAGE_CONVNEXT_SINGLE_V15_WORKER"
...
def _spawn_detached():
    ...
    log_path = LOG_DIR / f"convnext_single_v15_{ts}.log"
...
#   [v15] make_aligned_dataset.py 의 출력을 읽습니다.
#   구조:  <ALIGNED_DIR>/{train,validation,test}/{images,masks}/  +  csv/
ALIGNED_DIR   = BASE_DIR / "crop_hand_align"
HAND_CROP_DIR = ALIGNED_DIR
CSV_DIR       = ALIGNED_DIR / "csv"
MASK_ROOT     = ALIGNED_DIR                      # 마스크도 같은 트리에 있습니다
...
_require(HAND_CROP_DIR, "정렬 데이터셋 폴더 (make_aligned_dataset.py 를 먼저 실행하세요)")
_require(ALIGNED_DIR / "_DONE_hand.json",
         "정렬 완료 표식 (_DONE_hand.json). make_aligned_dataset.py 가 끝나지 않았습니다")
...
# ---------------------------------------------------------------- [v15 신규]
# 마스크 기반 배경 제거
#   근거: QC 시트에서 크롭 안에 판독 마커가 그대로 보입니다("L JMN", "L R", "BG").
#         이는 촬영 기관·기사마다 모양과 위치가 다른 표식이며, 기관마다 환자
#         연령 분포가 다르므로 모델이 뼈 대신 마커를 보고 나이를 맞히는
#         지름길을 배울 수 있습니다. RSNA 내부 val/test 는 같은 기관 분포라
#         이 지름길이 성능으로 드러나지 않고, 마커가 다른 배포처에서만 무너집니다.
#         마스크 밖 영역이 프레임의 약 60% 이고 그 전부가 마커·콜리메이션·촬영판
#         입니다. 골연령 정보는 0 이므로 지우는 것이 맞습니다.
#
#   부수효과: p1p99 퍼센타일을 마스크 안쪽 화소로만 계산하게 되어
#             강도 정규화 자체가 정확해집니다. 기존에는 X-ray 배경의 산란선이
#             (0 이 아니라 어중간한 회색이라) 통계에 그대로 섞였습니다.
USE_MASK_BG   = True     # 마스크 밖을 0 으로. False 면 v11 과 동일 동작
MASK_P1P99    = True     # p1p99 를 마스크 안쪽 화소로만 계산

# 마스크 열화 증강 (배포 대비)
#   배포처 세그멘테이션 품질이 학습만 못할 때를 대비합니다. 마스크에 의존하는
#   파이프라인은 마스크가 나빠지면 통째로 무너지므로 내성을 미리 학습시킵니다.
MASK_MORPH_P  = 0.30     # 마스크를 침식/팽창시킬 확률
MASK_MORPH_K  = (3, 11)  # 커널 크기 범위(홀수로 보정)
MASK_SKIP_P   = 0.05     # 마스킹 없이 학습할 확률 (마스크 실패 시 폴백 대비)
...
USE_AUG       = True
AUG_ROT_DEG   = 5           # [v15] 12 -> 5. 입력이 이미 정렬돼 있습니다
                            #   (make_aligned_dataset.py 잔차 중앙 0.0도).
                            #   0 으로 두지는 않습니다. 배포처에서 정렬이 실패했을 때
                            #   완전히 무너지지 않도록 최소한의 내성은 남깁니다.
...
PRE_TAG   = (f"raw{IMG_H}x{IMG_W}_{RESIZE_MODE}_pad{PAD_VALUE}_{PAD_ANCHOR}_n{NORM_MODE}"
             + ("_maskbg" if USE_MASK_BG else "") + ("_maskn" if MASK_P1P99 else ""))
...
CACHE_DIR = BASE_DIR / "cache_convnext_single_v15" / PRE_TAG
#   체크포인트는 헤드 shape 가 다르므로 반드시 분리 (스칼라 240-way 충돌 방지)
CKPT_DIR  = BASE_DIR / ("checkpoints_convnext_single_v15_ldl" if USE_LDL
                        else "checkpoints_convnext_single_v15_scalar")
...
print("=" * 72)
log("ConvNeXt-Tiny + LDL 기대값 회귀 골연령 시작 [v15]"
    if USE_LDL else "ConvNeXt-Tiny 스칼라 회귀 골연령 시작 [v15]")
...
log(f"[v15] 데이터   {ALIGNED_DIR}  (정렬 완료 데이터셋)")
log(f"[v15] 마스크   배경제거 {USE_MASK_BG} · p1p99 마스크내부 {MASK_P1P99} · "
      f"열화증강 p={MASK_MORPH_P} k={MASK_MORPH_K} · 마스킹생략 p={MASK_SKIP_P}")
log(f"[v15] 회전증강 {AUG_ROT_DEG}° (v11 12°, 입력이 이미 정렬됨)")
...
def norm_intensity(g, mode=None, mask=None):
    """이미지별 퍼센타일 스트레치.

    반드시 '패딩 전, 원본 크롭 위에서' 호출해야 합니다.
    letterbox 이후에 하면 패딩 0 이 통계에 섞여 스트레치가 망가집니다.

    [v15] mask 가 주어지면 마스크 안쪽 화소로만 퍼센타일을 계산합니다.
    기존의 g>0 조건은 '배경이 정확히 0' 이라는 가정에 기대는데, 실제 X-ray
    배경은 산란선 때문에 0 이 아니라 어중간한 회색이라 통계에 섞여 들어옵니다.
    마스크를 쓰면 손 조직만으로 계산되어 뼈-연부조직 대비가 최대로 펴집니다.
    """
    mode = NORM_MODE if mode is None else mode
    if mode == "none":
        return g
    if mask is not None and MASK_P1P99:
        v = g[(mask > 0) & (g > 0)]
        if v.size < 1000:                   # 마스크가 너무 작으면 기존 방식으로 폴백
            v = g[g > 0]
    else:
        v = g[g > 0]
...
def fit_canvas(g, h_out=None, w_out=None, mode=None,
               pad_val=None, anchor=None, norm=None, mask=None):
    ...
    g = to_gray8(g)
    #  [v15] 순서가 중요합니다. 마스크 통계로 정규화한 '뒤에' 배경을 지웁니다.
    #  먼저 지우면 0 이 된 배경이 g>0 폴백 조건과 얽혀 동작이 불투명해집니다.
    g = norm_intensity(g, NORM_MODE if norm is None else norm, mask=mask)
    if mask is not None and USE_MASK_BG:
        g = np.where(mask > 0, g, 0).astype(np.uint8)
    h, w = g.shape[:2]
...
def mask_path_for(split, img_path):
    """[v15] 이미지 경로에 대응하는 마스크 경로. 없으면 None."""
    d = MASK_ROOT / SPLIT_SUBDIR[split] / MASK_SUB
    for ext in (".png", ".PNG", ".jpg", ".jpeg", ".bmp", ".tif", ".tiff"):
        c = d / f"{Path(img_path).stem}{ext}"
        if c.exists():
            return c
    return None
...
PRE_INFO = {"norm_mode": NORM_MODE, "img_h": IMG_H, "img_w": IMG_W,
            ...
            "mask_bg": bool(USE_MASK_BG), "mask_p1p99": bool(MASK_P1P99),
...
def build_cache():
    ...
    for sp in SPLITS:
        ...
        for n, (_, r) in enumerate(df.iterrows(), 1):
            ...
            #  [v15] 마스크를 함께 읽어 정규화 통계와 배경 제거에 씁니다.
            #  마스크가 없으면 하드 실패시킵니다. 조용히 마스크 없이 넘어가면
            #  일부 표본만 다른 전처리를 받게 되어 원인 추적이 불가능해집니다.
            msk = None
            if USE_MASK_BG or MASK_P1P99:
                mp = mask_path_for(sp, r["path"])
                if mp is None:
                    failed += 1
                    n_mask_missing += 1
                    log(f"    [경고] 마스크 없음: {r['id']}")
                    continue
                msk = imread_kr(mp, cv2.IMREAD_GRAYSCALE)
                if msk is None:
                    failed += 1
                    n_mask_missing += 1
                    log(f"    [경고] 마스크 읽기 실패: {mp}")
                    continue
                if msk.shape[:2] != g.shape[:2]:
                    msk = cv2.resize(msk, (g.shape[1], g.shape[0]),
                                     interpolation=cv2.INTER_NEAREST)
                msk = (msk > 127).astype(np.uint8)

            canvas, info = fit_canvas(g, mask=msk)
            #  마스크도 같은 기하변환으로 캔버스에 맞춰 저장합니다.
            #  런타임 마스크 증강(침식/팽창)에 필요합니다.
            if msk is not None:
                mdst = CACHE_DIR / f"{sp}_mask" / f"{r['id']}.png"
                mdst.parent.mkdir(parents=True, exist_ok=True)
                mcanvas, _ = fit_canvas(msk * 255, norm="none")
                imwrite_kr(mdst, (mcanvas > 127).astype(np.uint8) * 255)
        ...
        log(f"  [{sp}] 생성 {made:,} · 스킵 {skipped:,} · 실패 {failed:,} "
            f"· 마스크없음 {n_mask_missing:,} ({time.time()-t0:.0f}s)")
        if n_mask_missing:
            raise SystemExit(f"[중단] {sp} 에서 마스크 {n_mask_missing:,}건이 없습니다. "
                             f"make_aligned_dataset.py 를 SAVE_MASK=True 로 다시 실행하거나 "
                             f"USE_MASK_BG/MASK_P1P99 를 False 로 두세요.")
    if pad_fracs:
...
# =========================================================================
def degrade_mask(m, rng=None):
    """[v15] 마스크 열화 증강. 배포처 세그멘테이션 품질 저하에 대한 내성.

    입력/반환 (H, W) uint8 0/1
    침식과 팽창을 같은 확률로 걸어 경계가 안팎 어느 쪽으로 어긋나도 견디게 합니다.
    """
    if rng is None:
        rng = np.random
    if rng.rand() >= MASK_MORPH_P:
        return m
    k = int(rng.randint(MASK_MORPH_K[0], MASK_MORPH_K[1] + 1)) | 1     # 홀수 보정
    ker = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (k, k))
    op = cv2.MORPH_ERODE if rng.rand() < 0.5 else cv2.MORPH_DILATE
    out = cv2.morphologyEx(m, op, ker)
    return out if out.sum() > 500 else m        # 너무 지워졌으면 원본 유지
...
class BoneAgeDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        #  [v15] 마스크 열화 증강.
        #  캐시본은 이미 배경이 제거된 상태입니다. 여기서 마스크를 다시 씌우는 게
        #  아니라, '마스크가 학습 때보다 나쁠 때' 를 모사하는 것이 목적입니다.
        #    침식 -> 손 가장자리가 잘려나간 경우
        #    팽창 -> 배경 일부가 남아 들어온 경우 (이때만 원본 화소가 필요)
        if self.split == "train" and USE_MASK_BG and MASK_MORPH_P > 0:
            mk = imread_kr(CACHE_DIR / f"{self.split}_mask" / f"{r['id']}.png",
                           cv2.IMREAD_GRAYSCALE)
            if mk is not None:
                if mk.shape[:2] != g.shape[:2]:
                    mk = cv2.resize(mk, (g.shape[1], g.shape[0]),
                                    interpolation=cv2.INTER_NEAREST)
                mk = (mk > 127).astype(np.uint8)
                if np.random.rand() < MASK_SKIP_P:
                    pass                            # 마스킹 실패 상황 모사: 그대로 사용
                else:
                    g = np.where(degrade_mask(mk) > 0, g, 0).astype(np.uint8)
...
ARCH = {"BACKBONE": BACKBONE, "IMG_H": IMG_H, "IMG_W": IMG_W,
        ...
        "USE_MASK_BG": USE_MASK_BG, "MASK_P1P99": MASK_P1P99,
