...
#
#   ▶ v9 = v7 대비 '전처리 최소화 + 백본 교체'
#     [제거] ⓪ 마커 억제      MARKER_MODE = "off"
#            - v7 에서 실제로 마커를 못 지웠고, 지워지면 99백분위가 내려가
#              정규화가 더 공격적으로 변해 포화를 키웠음
#     [제거] ⓪-2 회전 정렬    ALIGN_MODE = "off"
#            - PCA 주축이 불안정. warpAffine 검은 쐐기가 백분위/패딩/CAM 을 오염
#            - 회전 불변성은 AUG_ROT_DEG=12 증강으로 확보
#     [제거] ① 밝기 정규화    NORM_MODE = "none"
#            - 타이트 크롭에서 백분위가 조직 내부에 앉아 3.6배 과확장 -> 포화
#     [제거] ② Top-Hat        TOPHAT_MODE = "none"
#            - add 모드는 감산항이 없어 밝기가 단조 증가, 골단이 255 에서 절단
#     [유지] ③ CLAHE (리사이즈 후 512 에서 적용) -> 이제 유일한 대비 표준화 수단
#     [유지] ④ 비율유지 리사이즈  ⑤ 배경색 패딩 (왜곡 0)
#     [변경] 출력 크기 560 -> 512
#     [변경] Phase II 채널 = H + R1 + R2  (erased 는 cls-r2 학습 입력으로만 사용)
#     [변경] Phase II 백본 = EfficientNetV2-S
#     [유지] R1 밴드 CAM / R2 논문 erase 방식 / 성별 임베딩 / LDL 손실
#     [분리] cache_attention_v9/<PRE_TAG> / checkpoints_attention_v9 / logs/*_v9_*
#
#   ▶ 전처리 체인 (최종)
#       원본(YOLO 크롭) -> [uint8 변환] -> 비율유지 리사이즈 -> CLAHE -> 배경색 패딩
#
#   ▶ 실행: python attention_ldl_boneage_v9_effnetv2.py
...
#       --rebuild-cache  손 크롭 -> 512 리사이즈 캐시를 강제로 재생성
...
RUN_STATE = LOG_DIR / "boneage_attention_v9_running.json"
_WORKER_ENV = "BONEAGE_ATTENTION_V9_WORKER"
...
def _spawn_detached():
    ...
    log_path = LOG_DIR / f"boneage_attention_v9_{ts}.log"
...
CLS_SIZE_REGION = 512     # [v9] 512 -> InceptionV3 특징맵 약 14x14
                          #   (560 대비 CAM 격자가 약간 거칩니다)
REG_SIZE        = 512     # [v9] 560 -> 512 (메모리 -16%, 배치 여유)
...
AGG_CHANNELS = ("hand", "r1", "r2")   # [v9] erased -> r2
                                      #   erased 는 cls-r2 학습 입력 전용

# -- [v9] Phase II 백본 --------------------------------------------------
#   global_pool="" 로 특징맵을 받으므로 stride32 계열이면 무엇이든 교체 가능
#     convnext_tiny.fb_in22k_ft_in1k      : 28M, IN-22k 사전학습
#     tf_efficientnetv2_s.in21k_ft_in1k   : 22M, 고해상도 학습 효율
#     legacy_xception                     : v6/v7 기준선
REG_BACKBONE = "tf_efficientnetv2_s.in21k_ft_in1k"
...
# =========================================================================
# ① 밝기 정규화
NORM_MODE    = "none"         # [v9] 제거. 타이트 크롭에서 과확장 -> 포화
                              #   되돌리려면 "percentile"
NORM_LO, NORM_HI = 1.0, 99.0  # 백분위 하한/상한 (마커·핫픽셀 클리핑)

# ② Top-Hat  (White Top-Hat = 원본 - Opening(원본))
TOPHAT_MODE   = "none"    # [v9] 제거. add 는 감산항이 없어 골단이 255 절단
                          #   되돌리려면 "addsub" + WEIGHT 0.5
...
# =========================================================================
# ── ⓪ 마커 억제 ─────────────────────────────────────────────────────
MARKER_MODE = "off"           # [v9] 제거 (v7 에서 실제 동작 안 함)
...
# ── ⓪-2 회전 정렬 ───────────────────────────────────────────────────
ALIGN_MODE = "off"        # [v9] 제거 (쐐기가 통계/CAM 오염)
...
CACHE_DIR = BASE_DIR / "cache_attention_v9" / PRE_TAG
CKPT_DIR  = BASE_DIR / "checkpoints_attention_v9"
SPLITS    = ("train", "val", "test")
SUBDIRS   = ("hand512", "r1", "r2", "erased")
...
BASE_DONE = CACHE_DIR / "_DONE_hand512.json"
...
print("=" * 64)
log("Attention(R1/R2) + LDL 골연령 - v9 (EfficientNetV2-S) 시작")
...
def enhance(gray, return_k=False, return_info=False):
    """[v9] 마커/정규화/회전/Top-Hat 전부 off -> 사실상 uint8 변환만.
...
# =========================================================================
# [D] hand512 캐시 - 손 크롭을 512x512 으로 리사이즈해 1회만 저장
...
def build_hand_cache():
    ...
    for sp in SPLITS:
        ...
        for i, (_, r) in enumerate(df.iterrows(), 1):
            dst = CACHE_DIR / "hand512" / sp / f"{r['id']}.png"
            if dst.exists() and not REBUILD_CACHE:
                ...
                if g is None:
                    ...
                    made += 1
            if i % 2000 == 0:
                log(f"  hand512 {sp} {i}/{len(df)}")
        log(f"[hand512:{sp}] 생성 {made} | 스킵 {skipped} | 실패 {failed}")
    if ks:
        n_tot = len(ks)
...
if hand_cache_valid():
    log("hand512 캐시 유효 - 스킵")
else:
    log(f"hand512 캐시 생성: 정규화({NORM_MODE}) -> TopHat({TOPHAT_MODE}) -> "
        f"CLAHE({CLAHE_CLIP},{CLAHE_TILE}@{CLAHE_STAGE}) -> "
        f"scale({SCALE_MODE}) -> pad({PAD_MODE}) -> {REG_SIZE}px")
    build_hand_cache()
    log("hand512 캐시 완료")
...
for _sp in SPLITS:
    SPLIT_DFS[_sp] = filter_cached(SPLIT_DFS[_sp], "hand512", _sp)
train_df, val_df, test_df = SPLIT_DFS["train"], SPLIT_DFS["val"], SPLIT_DFS["test"]
...
class ClsDataset(Dataset):
    """Phase I 학습용. sub 로 hand512 / erased 중 어느 캐시를 쓸지 지정."""
...
if roi_cache_valid():
    ...
    m_r1 = train_cam_classifier("cls-r1", CLS_R1_BEST, CLS_R1_LAST, "gmp", "hand512")
...
def build_qc_sheet(n=N_QC, tag="qc_roi"):
    ...
    for rr, (_, r) in enumerate(samp.iterrows()):
        raw = imread_kr(r["hand_path"], cv2.IMREAD_GRAYSCALE)
        ref = imread_kr(CACHE_DIR / "hand512" / "val" / f"{r['id']}.png", cv2.IMREAD_GRAYSCALE)
...
KIND2SUB = {"hand": "hand512", "r1": "r1", "r2": "r2", "erased": "erased"}
...
def make_reg_backbone(pretrained=True):
    """[v9] Phase II 백본. REG_BACKBONE 한 줄로 교체됩니다.
...
# =========================================================================
# -- 하이퍼파라미터 -----------------------------------------------------------
BATCH_SIZE   = 10       # [v9] 512 입력 + EfficientNetV2-S. OOM 이면 낮추세요
EPOCHS       = 120     # [논문] 120 epoch
LR_STEPS     = [(60, 1e-4), (90, 3e-5), (120, 1e-5)]   # [v9] 논문 LR 의 1/3
                          #   (현대 백본은 3e-4 에서 발산하기 쉬움)
WEIGHT_DECAY = 0.05    # [v9] AdamW 표준값 (논문은 미언급)
...
if EVAL_ONLY:
    ...
    # [v9] Adam -> AdamW. 현대 백본은 decoupled weight decay 가 표준
    ...
    if AUTO_RESUME and LAST_CKPT.exists():
        ...
        if bad:
            ...
            log("  * 처음부터 다시 하려면 checkpoints_attention_v9 의 last.pt 삭제 후 실행")
    else:
