...
DEFAULT_CACHE_ROOT = BASE / "cache_convnext_single_v11"
...
def resolve_cache_val(explicit: str | None):
    """
    V11 validation cache 경로 자동 탐색.
    폴더명이 약간 달라도 raw768x512 / letterbox / mp1p99 키워드와
    val 또는 validation 하위 폴더를 찾아 사용한다.
    """
    if explicit:
        p = Path(explicit).resolve()
        if p.is_dir():
            return p
        raise FileNotFoundError(f"--cache_val 경로 없음: {p}")

    root = DEFAULT_CACHE_ROOT.resolve()

    if not root.is_dir():
        raise FileNotFoundError(f"V11 cache root 없음: {root}")

    candidates = []

    # 1) 직접적인 val / validation 폴더 탐색
    for p in root.rglob("*"):
        if not p.is_dir():
            continue

        name = p.name.lower()
        if name not in {"val", "validation"}:
            continue

        parent_text = str(p.parent).lower()

        score = 0
        for token in ("raw768x512", "letterbox", "mp1p99"):
        ...
        if n_img > 0:
            score += min(n_img, 5000) / 1000.0

        candidates.append((score, n_img, p))

    if not candidates:
        # 진단용으로 실제 하위 폴더 목록을 보여줌.
        children = []
        for p in root.iterdir():
            if p.is_dir():
                children.append(str(p))
        raise FileNotFoundError(
            "V11 validation cache 자동탐색 실패.\n"
            f"검색 root: {root}\n"
            "확인된 하위 폴더:\n"
            + "\n".join(f"- {x}" for x in children[:30])
        )
    ...
    print("[AUTO] V11 validation cache 후보:")
    for score, n_img, p in candidates[:10]:
        print(f"  score={score:.3f} images={n_img} | {p}")
...
def main():
    ...
    ap.add_argument(
        "--cache_val",
        default=None,
        help="생략 시 G:\\Project\\sinra_cho\\cache_convnext_single_v11 아래에서 V11 val cache 자동 탐색",
    )
    ap.add_argument(
        "--checkpoint",
        default=None,
    ...
    cache_dir = resolve_cache_val(args.cache_val)
