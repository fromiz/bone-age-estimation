# -*- coding: utf-8 -*-
# =========================================================================
# 손 X-ray 골연령 추정 - ConvNeXt-Tiny + LDL + FiLM 성별조건화  [v16]
#
...
# │            ▼                                                         │
# │      [F] 캐시  cache_convnext_single_v16/<PRE_TAG>/  ← v16 전용      │
# │            ▼                                                         │
...
#
# ▶ 실행: python convnexttiny_single_640x448_v16.py
...
#
# ▶ 산출물  checkpoints_convnext_single_v16_ldl/  (USE_LDL=False 면 _scalar)
...
#   logs/convnext_single_v16_<ts>.log   실행 로그 전체
# =========================================================================
...
RUN_STATE = LOG_DIR / "convnext_single_v16_running.json"
_WORKER_ENV = "BONEAGE_CONVNEXT_SINGLE_V16_WORKER"
...
def _spawn_detached():
    ...
    log_path = LOG_DIR / f"convnext_single_v16_{ts}.log"
...
IMG_H, IMG_W = 640, 448   # [v16] 768x512 -> 640x448 (화소 -27%)
...
CACHE_DIR = BASE_DIR / "cache_convnext_single_v16" / PRE_TAG
#   체크포인트는 헤드 shape 가 다르므로 반드시 분리 (스칼라 240-way 충돌 방지)
CKPT_DIR  = BASE_DIR / ("checkpoints_convnext_single_v16_ldl" if USE_LDL
                        else "checkpoints_convnext_single_v16_scalar")

#   [v16] 경로 오염 방지. v2/v3 때 캐시 디렉터리가 이전 버전을 가리켜
#   다른 데이터를 학습한 사고가 있었습니다. 문자열로 직접 검사합니다.
for _p, _n in ((CACHE_DIR, "CACHE_DIR"), (CKPT_DIR, "CKPT_DIR")):
    if "_v16" not in str(_p):
        raise SystemExit(f"[중단] {_n} 에 _v16 이 없습니다: {_p}")
    for _old in ("_v9", "_v10", "_v11", "_v12", "_v13", "_v14", "_v15", "_v17"):
        if _old in str(_p):
...
print("=" * 72)
log("ConvNeXt-Tiny + LDL 기대값 회귀 골연령 시작 [v16]"
    if USE_LDL else "ConvNeXt-Tiny 스칼라 회귀 골연령 시작 [v16]")
...
log(f"[v16] 캔버스 {IMG_H}x{IMG_W} (v11 768x512, 화소 "
    f"{100*(IMG_H*IMG_W)/(768*512)-100:+.0f}%)")
log(f"[v16] 데이터 {HAND_CROP_DIR}  (split/images 단층 탐색, masks 혼입 차단)")
...
def index_files(split):
    ...
    [v16] rglob 재귀 탐색을 glob 단층 탐색으로 바꿨습니다.
